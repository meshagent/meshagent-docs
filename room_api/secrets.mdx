...
> **CLI Note**: Room-scoped secrets and OAuth actions are under `meshagent room secrets`. For project wide or image-pull secrets, use `meshagent secrets`.
...
<CodeGroup>
```bash CLI
meshagent room secrets delete --room=myroom --delegated-to=participant_123 SECRET_ID
```
```python Python
await room.secrets.delete_secret(id="<secret_id>")
```
</CodeGroup>
