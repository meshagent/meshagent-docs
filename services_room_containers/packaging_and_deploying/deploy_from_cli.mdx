...
Use `meshagent process deploy` to create and deploy a service in one command. This is the fastest path for new process-backed agents. This command deploys a container-based service using the `meshagent-cli` image (not an external service).
...
This deploys a process-backed agent with web search and storage enabled. Run `meshagent process deploy --help` to see the other available flags for the agent. MeshAgent provides a variety of tools you can add to the agent out of the box. There are two flag styles:
