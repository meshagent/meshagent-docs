...
To deploy a service in MeshAgent, you write a YAML configuration file that describes what to run and how to connect it to a room. Services can be deployed with the MeshAgent CLI, from MeshAgent Studio, or by sharing a Powerboards install link.
...
## CLI Only Deployment or Custom Manifest?
...
meshagent service create-template --file meshagent.yaml --value key=value
```
...
- `private` — Keeps interactive container access scoped to the owning service. Most services should keep the default `true`. You would only change this if you intentionally want a more broadly shared interactive container workflow.
...
| `working_dir` | string | No | Working directory used when starting the container command. |
