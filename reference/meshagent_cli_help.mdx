...
* `webserver`: Run a webserver connected to a room
...
Run a webserver connected to a room
...
* `--key TEXT`: an api key to sign the token with
* `--file-route TEXT`: Expose a local folder as /route:/path (repeatable)
* `--python-route TEXT`: Mount a python handler as /route:/path.py (repeatable)
* `--host TEXT`: Host to bind the server  [default: 127.0.0.1]
* `--port INTEGER`: Port to bind the server  [default: 8000]
* `--watch`: Reload python routes when files change
* `--help`: Show this message and exit.
