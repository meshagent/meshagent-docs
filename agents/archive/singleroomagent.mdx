...
`SingleRoomAgent` does not dictate any particular behavior or communication style. Think of it as a room-aware shell that gives your agent a predictable lifecycle and a safe way to interact with other participants.

## When to Use It
- You need to join a room, run initialization logic, and stay connected for the room lifetime.
- You are building a specialized room-connected agent that does not fit the standard CLI process workflow.
...
- `title (str | None)`: Human-friendly display name. If omitted and you set `name`, it defaults to that value.
...
These arguments are stored on the instance and serialized through `Agent.to_json()` when a derived agent registers itself with the room service. The `name` field in that JSON is taken from the participant token, not the constructor.
...
- [Process Agents Overview](../process/overview): Interact with a conversational or multi-channel process-backed agent.
- [VoiceBot](../standard/voicebot): Interact with an agent through voice/speech.
- [Process Agents Overview](../process/overview): Add chat, mail, queue, or toolkit entry points to a process-backed agent.
