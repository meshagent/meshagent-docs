...
| --- | --- |
| `meshagent.agent.turn.start` | Start a new turn for a given thread path, carried on the process-message `thread_id` field, with typed input content. |
...
- `thread_id`: the persisted thread path this work belongs to
...
Queue and scheduled-task inputs use `path` for the same value. The process runtime carries that resolved thread path as `thread_id` after the queue item has been converted into a turn-start message.
