...
A ``VoiceBot`` combines speech-to-text (STT), text-to-speech (TTS), voice activity detection (VAD), and LLM-based reasoning into a single adaptive experience. It can greet users, run tools, and respond through natural speech — all without extra setup.
