...
- **.NET**: use the CLI or the generic invoke API (`room.Invoke(...)`) for memory operations.
...
These examples use the Python `MemoryClient` API. The CLI, Dart SDK, and TypeScript SDK expose matching room-memory operations for the same methods.
