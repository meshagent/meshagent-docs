...
`meshagent setup` signs you in, or if you are already signed in lets you continue with the current account or switch accounts, then lets you choose or create a project and activates the project for the CLI. If the Codex CLI is installed, setup can also optionally reuse an existing MeshAgent Codex profile for the active project or create a new one after the project is activated. After this completes, the CLI is ready to create rooms, run agents, and deploy services in the active project.
...
- [Packaging and Deploying Services](../services/deployment/deploy_services): go deeper on manifests, permissions, and deployment
