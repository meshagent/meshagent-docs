...
* `codex`: Join a Codex-backed agent to a room
...
* `--focus TEXT`: Project focus for non-interactive use. Use stable IDs: webserver (Web App), backend-agent (Agent Toolkit), chatbot (OpenAI Chatbot), chatbot-anthropic (Anthropic Chatbot), chatbot-ui (Agent UI), room-chat (Room Chat), meeting-app (Meeting App), or contact-form (Contact Form).
...
```console
$ meshagent session traces [OPTIONS] [SESSION_ID]
```
...
* `--model TEXT`: Name of the LLM model to use  [default: gpt-5.5]
...
```console
$ meshagent webhook get [OPTIONS] WEBHOOK_ID
```
...
```console
$ meshagent agent secret [OPTIONS] COMMAND [ARGS]...
```
...
```console
$ meshagent agent secret list [OPTIONS]
```
...
```console
$ meshagent agent secret get [OPTIONS] SECRET_ID
```
...
* `--id TEXT`: Agent ID (preferred)
* `--name TEXT`: Agent name
* `-o, --output TEXT`: output format   [default: json]
* `--help`: Show this message and exit.
...
```console
$ meshagent agent secret set [OPTIONS] SECRET_ID
```
...
* `--id TEXT`: Agent ID (preferred)
* `--name TEXT`: Agent name
* `--secret-name TEXT`: Display name
* `--type TEXT`: Secret content type  [default: keys]
* `--data TEXT`: Secret data as a JSON object, for example &#x27;&#123;&quot;OPENAI_API_KEY&quot;:&quot;...&quot;&#125;&#x27;  [required]
* `--help`: Show this message and exit.
...
```console
$ meshagent agent secret delete [OPTIONS] SECRET_ID
```
...
* `--offset INTEGER RANGE`: Offset for pagination  [default: 0; x&gt;=0]
* `--order-by TEXT`: Order by column (e.g. &quot;agent_name&quot;, &quot;created_at&quot;)  [default: agent_name]
...
* `--count INTEGER`: Maximum number of mailboxes to return  [default: 100]
...
* `--model TEXT`: Name of an LLM model to make available. Can be repeated.  [default: gpt-5.5]
...
* `--thread-storage [meshdocument|dataset|none]`: Thread storage backend for process agents.  [default: meshdocument]
...
* `--max-output-tokens INTEGER`: Maximum output tokens to request from OpenAI Responses models.  [default: 32000]
* `--reasoning-effort TEXT`: Reasoning effort to request from OpenAI Responses models.
* `--channel TEXT`: Attach a channel to the agent process. Can be repeated. Currently supported: chat, mail:EMAIL_ADDRESS, queue:QUEUE_NAME, toolkit:NAME, websocket:PORT, websocket://HOST:PORT.
...
Join a Codex-backed agent to a room
...
* `join`: Join a room and run a Codex-backed agent.
* `service`: Add a Codex-backed agent service to the host.
* `spec`: Generate a service spec for deploying Codex.
* `deploy`: Deploy a Codex-backed agent service.
* `run`: Join a room, run Codex, and wait for...
* `threads`: List threads from the local Codex app-server.
* `messages`: List messages in a Codex-backed agent thread.
* `grep`: Search coalesced messages in a...
* `use`: Send a one-shot or interactive message to...

### `meshagent codex join`

Join a room and run a Codex-backed agent.

**Usage**:

```console
$ meshagent codex join [OPTIONS]
```
...
* `--agent-name TEXT`: Name of the agent to call
* `--token-from-env TEXT`: Name of environment variable containing a MeshAgent token
* `-m, --model TEXT`: Default Codex model
* `--codex-bin TEXT`: Path to a Codex binary. Defaults to the bundled pinned runtime.
* `--codex-config TEXT`: Pass a Codex config override as key=value. Can be repeated and is forwarded to codex app-server.
* `--working-dir TEXT`: Working directory for Codex turns.
* `--key TEXT`: an api key to sign the token with
* `--thread-dir TEXT`: Thread directory for dataset thread storage namespaces.
* `--thread-storage [none|codex|dataset]`: Thread storage backend: none, codex, or dataset.  [default: codex]
* `--channel TEXT`: Attach a channel to the Codex agent. Can be repeated. Currently supported: chat, websocket:PORT, websocket://HOST:PORT.
* `--websocket-auth [jwt|iap|none]`: Authentication mode for websocket channels: jwt, iap, or none.  [default: jwt]
* `--help`: Show this message and exit.

### `meshagent codex service`

Add a Codex-backed agent service to the host.

**Usage**:

```console
$ meshagent codex service [OPTIONS]
```
...
### `meshagent codex spec`

Generate a service spec for deploying Codex.

**Usage**:

```console
$ meshagent codex spec [OPTIONS]
```
...
### `meshagent codex deploy`

Deploy a Codex-backed agent service.

**Usage**:

```console
$ meshagent codex deploy [OPTIONS]
```
...
Join a room, run Codex, and wait for messages.

**Usage**:

```console
$ meshagent codex run [OPTIONS]
```
...
* `--websocket-auth [jwt|iap|none]`: Authentication mode for websocket channels: jwt, iap, or none.  [default: jwt]
* `--verbose`: Enable verbose logging and disable default log suppression
* `--user TEXT`: User name for the local websocket run client.  [default: you]
* `--thread-id TEXT`: Codex thread id to open
* `--message TEXT`: the input message to use
* `--help`: Show this message and exit.

### `meshagent codex threads`

List threads from the local Codex app-server.

**Usage**:

```console
$ meshagent codex threads [OPTIONS]
```
...
### `meshagent codex messages`

List messages in a Codex-backed agent thread.

**Usage**:

```console
$ meshagent codex messages [OPTIONS]
```
...
### `meshagent codex grep`

Search coalesced messages in a Codex-backed agent thread.

**Usage**:

```console
$ meshagent codex grep [OPTIONS] PATTERN
```
...
Send a one-shot or interactive message to a running Codex-backed agent.

**Usage**:

```console
$ meshagent codex use [OPTIONS]
```
...
* `--thread-id TEXT`: Codex thread id to open
* `--message TEXT`: the input message to use
* `--websocket-url TEXT`: Connect to a websocket channel instead of room chat.
* `--websocket-auth [jwt|iap|none]`: Authentication mode for --websocket-url: jwt, iap, or none.  [default: iap]
* `--user TEXT`: User name for the websocket use client.  [default: you]
* `--help`: Show this message and exit.
...
* `secret`: Manage room-scoped secrets and OAuth flows
...
```console
$ meshagent room secret exists [OPTIONS]
```
...
* `-e, --env TEXT`: Set environment variable as KEY=VALUE
* `--env-secret TEXT`: Set environment variable from a room secret as NAME=SECRET_ID
* `--identity TEXT`: Identity name to use for the connected token, --meshagent-token, and --env-secret. Required with --role, --meshagent-token, and --env-secret. When set, room connect mints a participant token locally.
* `--role TEXT`: Participant role for locally minted tokens. Requires --identity and defaults to agent.
* `--meshagent-token TEXT`: Inject MESHAGENT_TOKEN using userDefault, agentDefault, full, or a JSON ApiScope object.
* `--template TEXT`: Allowed values: agent, none. agent: MeshAgent sets MESHAGENT_TOKEN, OPENAI_API_KEY, and ANTHROPIC_API_KEY to a room-scoped MeshAgent token, and sets OPENAI_BASE_URL, ANTHROPIC_BASE_URL, MESHAGENT_API_URL, MESHAGENT_PROJECT_ID, and MESHAGENT_ROOM from the connected room unless manually set. none: MeshAgent applies no template defaults.  [default: agent]
* `--help`: Show this message and exit.
