...
After you build an image and author a ``Service`` manifest, deploy it so MeshAgent automatically starts the service in every room. See the [project service configuartion docs](./configuartion) for more details on creating the ``Service`` manifest and Dockerfile for your service. 
...
- Any applicable [secrets and image pull secrets](../secrets/secrets) for running the service 
...
- Learn how to [configure](../room_containers/configuration) and [run](../room_containers/running) a service in a specific MeshAgent Room using the Containers API. See a complete [room service container example here](../room_containers/room_containers_example).
