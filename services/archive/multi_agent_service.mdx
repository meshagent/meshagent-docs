...
After seeding the files, the script runs `meshagent multi join`, which starts multiple agents in one container. The `-c` flag takes a semicolon-separated list of agent configurations. Each agent definition specifies its type, name, tools it needs access to (e.g., `--web-search`, `--require-toolkit=email`), its system prompt (`--rule`), and optionally room-editable rules files (`--room-rules`). In this example, all three agents also use the scoped memory path `agents/news-reporter/memories`, the chatbot uses `--threading-mode=default-new --thread-dir=/agents/news-reporter/threads`, and the worker uses `--threading-mode=auto` with the same `--thread-dir`, so they publish into the same thread list.
