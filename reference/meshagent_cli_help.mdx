...
* `--focus TEXT`: Project focus for non-interactive use. Use stable IDs: webserver (Web App), backend-agent (Agent Toolkit), chatbot (OpenAI Chatbot), chatbot-anthropic (Anthropic Chatbot), chatbot-ui (Agent UI), room-chat (Room Chat), or contact-form (Contact Form).
...
* `--thread-storage [meshdocument|meshagent|dataset|none|codex]`: Thread storage backend for process agents.  [default: meshdocument]
