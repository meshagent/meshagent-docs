...
For most new text and background workflows, the recommended starting point is [`meshagent process`](../process/overview), which lets one agent handle multiple entry points such as chat, mail, queues, and toolkit calls.
...
### Recommended Starting Point: `meshagent process`
For most new text and background workflows, start with [`meshagent process`](../process/overview).
...
This is the clearest path when one workflow may need several entry points over time. Instead of splitting the same logic across separate chat, mail, queue, and callable agents, you can keep one agent identity and attach the channels you need.

This is also the direction MeshAgent is moving for many non-voice agent patterns.

Today, `VoiceBot` is still separate because `meshagent process` does not currently expose a voice channel.

See [Multi-channel Agents Overview](../process/overview) and [Using Multi-channel Agents](../process/using_process) for the recommended workflow.

### Specialized Agent Types
MeshAgent also includes focused agent types that remain useful when you want a dedicated single-purpose runtime or are working with an existing codebase that already uses them.

### Participant-Based Agents
Interactive agents you communicate with via voice or text. Best when you need a conversational interface and interactive, iterative problem-solving with the end-user. These agents:
...
* **[ChatBot](./chatbot):** Conversational text agent. Best when you want a dedicated chat-focused runtime.
* **[VoiceBot](./voicebot):** Conversational speech and voice agent. Integrates with LiveKit for audio in and out.

### Background Agents
Background agents triggered by tasks, queues, or other events to execute specific tasks. Best when you need a dedicated single-surface background runtime or a specific SDK base class. These agents:
...
* **[TaskRunner](./taskrunner):** Schema-driven background agent. Define input and output schemas and an `ask()` method.
* **[Worker](./worker):** Queue-driven background agent. Listens for messages and processes them sequentially.  
* **[MailBot](./mailbot):** Specialized worker for email workflows. 
...
- **`Agent`** – Base identity, metadata, and `AgentSessionContext`.  
- **`SingleRoomAgent`** – Binds an agent to a Room connection and installs required toolkits.  
- **Specialized Base Agents** – These extend `SingleRoomAgent` and are either participant-oriented (ChatBot, VoiceBot) or run in the background (TaskRunner, Worker, Indexer, MailBot). 
...
| [Project Service](../../services_room_containers/packaging_and_deploying/packaging)     | Deploy once to run automatically in every room of a project (admin only).  | Always-on shared agents            |
| [Room Service](../../services_room_containers/packaging_and_deploying/packaging)        | Deploy to a single Room only. Other Rooms in the Project won't see it.  | Agents that should stay room-scoped. |

## Managing Agents via the Room Agents API
There are three first-class ways to coordinate work between agents: **messages**, **queues**, and **tools**. Messages give you one-way messaging, queues give you async one-way messaging, and tools provide request/response-style RPC.

For messages and queues, use the [Messaging API](../../room_api/messaging) and [Queues API](../../room_api/queue). The Room Agents API lets you list and invoke available tools from code. This includes TaskRunners, which expose a tool that uses an LLM to accomplish a task.

For more details check out the [Room Agents documentation](../../room_api/agents).

| Method | Purpose |
| --- | --- |
| [Call](../../room_api/agents#call) | Invites an external service (your agent) to join the room. |
| [List Toolkits](../../room_api/agents#list-toolkits) | Retrieves all toolkits (and their tools) available in the room. |
| [Invoke Tool](../../room_api/agents#invoke-tool) | Directly executes a specific tool within a toolkit without routing through an agent. |


## Next Steps
- [SingleRoomAgent](./singleroomagent): Understand the different types of agents in MeshAgent and the core classes for each. 
- [Multi-channel Agents Overview](../process/overview): Learn when to use the recommended multi-channel agent path.
- [Using Multi-channel Agents](../process/using_process): Start with the practical `meshagent process` workflow.
- [Build a Chat Agent](./chatbot): Learn how to use and customize a dedicated ChatBot runtime.
