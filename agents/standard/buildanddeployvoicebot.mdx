...
## Overview
The MeshAgent **VoiceBot** lets you add a voice agent to any MeshAgent Room with just a few lines of code! VoiceBots use the *same toolkits* as ChatBots, so everything you learned in the [build and deploy a chatbot quick-start](meshagent-sdk/meshagent-docs/agents/standard/buildanddeployvoicebot.mdx) still applies—your agent simply talks instead of types.
...
## Getting Started
*Note: These steps are identical to the [Build and Deploy a ChatBot](agents/standard/buildanddeploychatbot.mdx) steps. Feel free to skip if you are already setup.*
...
2. Call the voicebot into the room 
...
Now we can navigate to the [MeshAgent Studio](https://studio.meshagent.com/) and we'll see our agent, "myvoiceagent" show up under the messaging tab! We can talk to the agent now and make sure that it responds with a fun fact like we instructed it to.
...
This is the same process described in the [Build and Deploy a ChatBot Example](agents/standard/buildanddeploychatbot.mdx).
