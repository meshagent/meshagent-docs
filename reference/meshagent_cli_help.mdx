...
* `--pack-architecture TEXT`: Architecture metadata for the packed build context image. Defaults to the local machine architecture.
* `--pack-room-path TEXT`: Room storage path for the uploaded packed archive. Defaults to &#x27;/.images/&#123;manifest-digest&#125;.tar&#x27;. If a directory is provided, &#x27;&#123;tag&#125;.tar&#x27; is appended.
...
* `--project-id TEXT`: A MeshAgent project id. If empty, the activated project will be used.
* `--room TEXT`: Room name
* `-o, --output TEXT`: Local path to write the OCI archive tar  [required]
* `--base TEXT`: Optional base image reference. Defaults to scratch semantics.
* `--architecture TEXT`: Architecture to use when resolving --base  [default: amd64]
* `--room-path TEXT`: Room storage path to upload the archive to when --room is set. Defaults to the output file name.
* `--help`: Show this message and exit.
