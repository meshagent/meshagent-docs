...
Queue-backed process work also does **not** have to use threads. A queue channel can process jobs with fresh context each time, or it can write the work into thread documents so jobs have durable history.
...
Here is a simple `meshagent process` command that enables a thread-aware chat UI and stores its thread documents under a dedicated thread directory:
...
- [Process Agents Overview](./process/overview): choose chat, mail, queue, or toolkit entry points
