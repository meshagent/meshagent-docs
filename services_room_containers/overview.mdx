...
You can test agent and service configurations locally, then deploy them with MeshAgent so they’re available in rooms when your app (or your users) need them.
...
A `Service` manifest is a concrete spec that MeshAgent can run immediately. A `ServiceTemplate` is parameterized and needs user-supplied values to render a `Service` first.

- Use `kind: Service` when configuration is fixed (CI/CD deploys, always-on services).
- Use `kind: ServiceTemplate` when you want users or apps to supply inputs like API keys, prompts, or settings at deploy time.

Both can be used for project services or room services. The difference is whether configuration is fixed or provided at runtime. For full details, see [Packaging and Deploying Services](./packaging_and_deploying/packaging).
...
## Next Steps
- [Process Agents Overview](../agents/process/overview) to run and test a process-backed agent locally.
- [Manage secrets and registry access](../secrets/secrets_overview) to inject credentials securely.
- [Packaging and deploying services](./packaging_and_deploying/packaging) when you’re ready to deploy across every room.
