...
The payload is a JSON object. For MeshAgent queue consumers, that can include the same structured `prompt`, `content`, and `path` fields supported by `meshagent room queue send`.
...
queue:
  name: support-jobs
  payload:
    path: dataset://threads/support/{YYYY}/{MM}/{DD}/{HH}/summary
    prompt:
      - type: file
