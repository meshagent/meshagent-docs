...
1. **[Using the MeshAgent CLI](#phase-1-talk-with-the-built-in-voicebot-from-the-cli)**: Bring a ready-made VoiceBot into a room using a single CLI command.
3. **[Basic VoiceBot](#phase-2-building-a-simple-voicebot)**: A simple voice agent with a system prompt (rules)
3. **[VoiceBot with Built-in MeshAgent Tools](#phase-3-adding-built-in-meshagent-tools-to-our-voicebot)**: Extend our voice agent with prebuilt tools to interact with the user and write documents to the room.
4. **[VoiceBot with Built-in MeshAgent Tools and Custom Tools](#phase-4-adding-custom-tools-to-our-voicebot)**: Add our own tools to the agent for use-case specific tasks. 
