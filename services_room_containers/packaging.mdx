...
Packaging a service requires creating a YAML configuration file that tells MeshAgent how to run your code. MeshAgent can run your code as a Project Service (available in every room) or a Room Service (scoped to a specific room). This guide covers how to create these configuration files for both containerized services and external services.

## What You'll Learn
In this guide, you'll learn how to:
- Choose between `Service` and `ServiceTemplate` configurations
- Package containerized services that run in MeshAgent
- Register external services that run elsewhere
- Configure agents, endpoints, storage, secrets, and advanced features

##  Service vs ServiceTemplate: Which Should You Use?
The choice between `Service` and `ServiceTemplate` depends on whether users need to customize your service at deployment time.

### Use `Service` (kind: Service) when:
- Configuration is fixed and doesn't need user input
- You're deploying via CI/CD with predetermined values
- The service doesn't require runtime customization

### Use `ServiceTemplate` (kind: ServiceTemplate) when:
- Users need to provide their own API keys, prompts, or settings
- You're packaging a service for others to customize in Powerboards (The Powerboards UI allows users to dynamically supply the required variables for their service before it's created)
- Different deployments need different configurations

**Key difference**: A `ServiceSpec` runs as-is. A `ServiceTemplateSpec` collects user inputs, then MeshAgent calls `to_service_spec()` to generate the actual `ServiceSpec` that runs.
...
#### Scheduled Tasks
You can schedule queued messages by adding a `meshagent.agent.schedule` annotation to an agent. The value is a JSON string that defines a cron or one-time schedule and the message to enqueue.

```yaml yaml
agents:
  - name: reporter
    description: "Daily scheduled worker"
    annotations:
      meshagent.agent.type: "Worker"
      # Runs daily at 09:00 (cron timezone depends on scheduler settings).
      meshagent.agent.schedule: '{"name":"DailyReport","schedule":"0 9 * * *","queue":"reports","payload":{"topic":"ai"}}'
```

For programmatic management and schedule details, see [Scheduled Tasks](../rest_api/overview#scheduled-tasks).
...
Example token-backed environment variable:
```yaml yaml
environment:
...
#### Storage
Use the ``storage`` field to mount room, project, or image storage into your container.
...
| `image` | string | No | Source image for `storage.images` mounts |

Example image mount:
```yaml yaml
storage:
  images:
    - image: "registry.example.com/data:latest"
      path: /opt/data
      subpath: "models"
      read_only: true
```

**How image mounts work**
- The container still starts from `container.image` (your runtime image). Any command/entrypoint must exist in that runtime image.
- `storage.images` mounts files from another image into the running container as a read-only filesystem.
- `path` is the mount target; `subpath` (optional) points to a folder inside the mounted image.
- The mounted image must be pullable by the room runtime (public or with `pull_secret`).
...
Variables define what values users can provide when a container-based service launches. You can restrict inputs or hide sensitive ones. Template substitutions use Jinja; you can use `{{ variable_name }}` placeholders in any string field.
