...
For agents that should always have specific capabilities, pass the tools directly via the agent's `toolkits` parameter.  This works for both MCP tools and MeshAgent toolkits. See [Using OpenAI Connectors & MCP Servers](./openai_connectors_mcp_servers) for examples of creating agents with always-on MCP toolkits.
...
## Related Topics
- [Using OpenAI Connectors & MCP Servers](./openai_connectors_mcp_servers): Learn how to create agents with always-on toolkits
