...
| `path` | string | No | Optional thread path or template. Time tokens such as `{year}`, `{month}`, `{day}`, `{hour}`, and `{minute}` are expanded at runtime. Token matching is case-insensitive, so `{YYYY}/{MM}/{DD}/{HH}/{mm}` also works. |
...
agents:
  - name: assistant
    ...
    heartbeat:
      ...
      path: /agents/assistant/threads/heartbeats/{year}/{month}/{day}/{hour}.{minute}.thread
      prompt:
        - type: file
