...
```console
$ meshagent team [OPTIONS] COMMAND [ARGS]...
```
...
### `meshagent team service`

**Usage**:

```console
$ meshagent team service [OPTIONS]
```
...
* `--host TEXT`
* `--port INTEGER`
* `-c TEXT`: a list of commands to run, seperated by pipes
* `--help`: Show this message and exit.
...
```console
$ meshagent team add [OPTIONS] MODULE
```
...
* `--host TEXT`
* `--port INTEGER`
* `--path TEXT`: A path to add the service at
* `--identity TEXT`: The desired identity for the service
