...
Service-account API keys authenticate the service account; they do not bypass room or agent policy. Tokens requested normally use the service account's effective resource role. A service account with `participant_token_creator` can pass `user_id` when connecting to a room or agent, or when requesting a participant token, to act for that user. MeshAgent still derives the token permissions from the target user's effective role on the requested resource.
