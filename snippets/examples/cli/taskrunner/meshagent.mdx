...
container:
  ...
  command: "/usr/bin/meshagent/meshagent task-runner service --agent-name=taskrunner --web-search --storage --room-rules='agents/taskrunner/rules.txt' --rule='You are a helpful assistant'"
  storage:
    room:
