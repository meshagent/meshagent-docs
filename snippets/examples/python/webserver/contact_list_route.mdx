...
def _render_contacts(contacts: list[dict]) -> str:
    if not contacts:
        return "<p class=\"empty\">No contacts yet. Submit the contact form to add one.</p>"
    ...
    for contact in contacts:
        ...
        message = (
            escape(str(contact.get("message", "")).strip())
  ...
  <p>{message}</p>
    ...
    return "<section class=\"items\">{items}</section>".format(
        items="\n".join(items)
    )
