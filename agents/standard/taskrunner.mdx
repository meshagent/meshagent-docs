...
1. **Built-in instances** - `meshagent.runner` and `meshagent.schema_planner` are pre-deployed in every room
2. **CLI** - Launch your own instance via `meshagent task-runner join` with custom tools/rules  
3. **SDK** - Extend `LLMTaskRunner` or implement `TaskRunner` for custom logic

The `LLMTaskRunner` uses a fixed output schema defined at initialization (default: text response). For per-request structured output, use `meshagent.schema_planner` and pass an `output_schema` with each call.
...
- `meshagent.schema_planner` - Accepts a prompt and an `output_schema`, returns structured JSON matching that schema.
...
This example shows how to create your own `LLMTaskRunner` instances using `ServiceHost`. These work like the built-in `meshagent.runner` and `meshagent.schema_planner`.
...
From a different tab in your terminal invoke the `LLMTaskRunner` from the CLI: 
...
- `LLMTaskRunner` is a concrete implementation that calls an LLM adapter, resolves toolkits, and returns the model output.
