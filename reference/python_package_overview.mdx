...
| `meshagent-agents` | `meshagent.agents` | Agent classes such as `TaskRunner`, `Worker`, and `ChatBot` |
...
### ChatBot
The ``ChatBot`` is a conversational agent derived from the ``SingleRoomAgent``. It wires an LLMAdapter, optional tools, and manages chat threads for each user. Check out the [ChatBot](../agents/standard/chatbot) documentation to learn how to create and customize a chat based agent in MeshAgent. You can set custom rules for the agent and extend it's abilities with tools. 
