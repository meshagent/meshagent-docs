...
import CliWorkerYaml from "/snippets/examples/archive/cli/worker/meshagent.mdx"
import QueueWorker from "/snippets/examples/archive/python/deployable/queueworker/queueworker.mdx"
import PushQueue from "/snippets/examples/archive/python/deployable/queueworker/push_queue.mdx"
import ScratchDockerfile from "/snippets/examples/python/deployable/Dockerfile.mdx"
import QueueWorkerYaml from "/snippets/examples/archive/python/deployable/queueworker/meshagent.mdx"

> **Archived:** The dedicated `meshagent worker` CLI runtime is no longer the recommended path for new docs. Use [`meshagent process`](../process/overview) with a `queue` channel for new queue-driven agents. This page remains as historical reference for the older Worker-specific runtime.
...
If you need an interactive assistant, use [ChatBot](./chatbot). For real-time speech, see [VoiceBot](../standard/voicebot). For a callable function/agent pattern with structured schemas, see [TaskRunner](./taskrunner).
...
meshagent worker join --room quickstart --agent-name worker --queue tasks --storage \
...
Workers can also be extended with rules, tools, and skills for repeatable background workflows. See [Tools and Toolkits](../tools/tools_and_toolkits) and [Skills Overview](../../agent_skills/overview).
...
```bash bash 
meshagent worker deploy --room quickstart --service-name worker --agent-name worker --queue tasks --storage \
  --room-rules "agents/worker/rules.txt" --rule "Process queued tasks and save results to storage."
```
...
```bash 
meshagent worker spec --service-name worker --agent-name worker --queue tasks --storage \
  --room-rules "agents/worker/rules.txt" --rule "Process queued tasks and save results to storage."
```
...
To deploy your SDK Worker permanently, you'll package your code with a `meshagent.yaml` file that defines the service configuration and a container image that MeshAgent can run.
For full details on the service spec and deployment flow, see [Packaging and Deploying Services](../../services/deployment/deploy_services). If you want to bundle reusable private skills with the worker, see [Bundle Private Skills with a Custom Service](../../agent_skills/packaging_and_deploying).
...
2. Build context and tools. Create a fresh chat context (``init_session()``), apply any rules, and resolve toolkits from requires plus local toolkits.
...
- [VoiceBot](../standard/voicebot) for conversational speech/voice based agents
...
- [Skills Overview](../../agent_skills/overview) for reusable workflow guidance shared across agent runs
