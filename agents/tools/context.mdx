...
| ------------- | -------------- |
| `JsonResponse` | Structured JSON output (e.g., API payloads, summarised data). |
...
- `execute(context, name, arguments, optional attachment)`:
    - For a `RequestTool`, combines the header and payload into a `Request` object, invokes the tool, then returns the response.
...
Public toolkits are visible to any participant with `use_tools`. Private toolkits are only listed for the participant that registered them. Adjust these flags via [API scope grants](../../room_api/api_scopes) to decide which services or users can call tools in each room.
