...
## Overview
The MeshAgent **VoiceBot** lets you add a voice agent to any MeshAgent Room with just a few lines of code! VoiceBots use the *same toolkits* as ChatBots, so everything you learned in the [build and delpy a chatbot quick-start](agents/standard/buildanddeploychatbot.mdx) still applies—your agent simply talks instead of types.
