...
5. ChatBot resolves this turn’s toolkits: it collects `ToolkitBuilder`s from `get_thread_tool_providers(...)`, merges per-turn `ToolkitConfig`s (from MeshAgent Studio/CLI/App UI), calls `make_tools(...)` to create any dynamic toolkits, and combines the result with any static toolkits.
...
| `async def get_thread_tool_providers(thread_context, participant)` | Returns the `ToolkitBuilder` list the UI can offer for message-level tool selection.         |
