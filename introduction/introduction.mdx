...
Any conversational agents you deploy with MeshAgent can be used immediately in [Powerboards](../interfaces/powerboards), an open-source AI-native app built entirely on MeshAgent. Use it to share agents today, or fork it into your own MeshAgent-powered application. See [Share Agents in Powerboards](../services_room_containers/share_agents_in_powerboards) for deployment details.
...
- Register **[agents](../agents/standard/intro)** as first-class participants for chat, voice, background work, and tool-driven workflows
- Register **[tools](../agents/tools/intro)** (built-in, custom, MCP) and invoke them programmatically
- Invite teammates to collaborate live from [MeshAgent Studio](../interfaces/meshagent_studio), [Powerboards](../interfaces/powerboards), or your own application

### Built-in Room APIs
Rooms include [built-in capabilities](../room_api/overview) that remove infrastructure glue work:
...
- **[Memory API](../room_api/memory)** — build room-scoped structured memory and recall context across interactions
...
- Process-backed agents for chat, mail, queues, toolkit-style calls, and voice workflows
...
- Mail-capable process agents + document intelligence: process attachments, categorize and query documents, generate insights, and enrich context with research tools
...
Key workflows:
- **Start quickly with the CLI** — use [`meshagent process`](../agents/process/overview) to run a process-backed agent locally
...
- an in-room [LLM proxy](../agents/routing/llm_proxy) that secures API keys and provides automatic cost attribution and controls

---

## Powerboards: an open-source distribution and collaboration surface

[Powerboards](../interfaces/powerboards) is an open-source AI-native application built entirely on MeshAgent. See [Share Agents in Powerboards](../services_room_containers/share_agents_in_powerboards).
...
| **[MeshAgent Studio](../interfaces/meshagent_studio)** | [Browser workspace](https://studio.meshagent.com) + CLI for prototyping, testing tools, evaluating agents, and inviting teammates into live Rooms. | Multiplayer development: iterate fast with stakeholders and teammates in the same live Room. |
...
| **[Powerboards](../interfaces/powerboards)** | Open-source AI-native app built on MeshAgent for sharing and collaborating with deployed agents. | Use as-is for distribution, or fork into your own MeshAgent-powered application surface. |
...
| --- | --- | --- |
| **[Python](https://pypi.org/project/meshagent/)** | Room API, tools, voice support, agent building blocks, and legacy compatibility classes | Authoring and hosting production integrations and custom services |
