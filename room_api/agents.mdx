...
<Tabs>
  ...
  - **`supports_tools`** *(optional; defaults to `False`)*: Indicates whether the agent can use external tools.
  ...
  - **`requires`** *(optional)*: A list of `Requirement` objects (e.g., `RequiredToolkit`, `RequiredSchema`).
  - **`supportsTools`** *(optional; defaults to `False`)*: Indicates whether the agent can use external tools.
  - **`labels`** *(optional)*: Tags associated with the agent.
