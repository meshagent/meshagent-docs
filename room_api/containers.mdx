...
If you want a deployed service to use the Containers API, its participant token needs the appropriate Room API grants. See [API Scopes](../rest_api/api_scopes) and [Packaging and Deploying Services](../services_room_containers/packaging_and_deploying/packaging).

## CLI and SDK availability

- **CLI**: image and container lifecycle commands are available under `meshagent room container ...`.
- **Python, JavaScript/TypeScript, and Dart**: the current helper surface covers image, run/exec/log, and lifecycle operations.
- **.NET**: does not currently expose a `ContainersClient` helper.

JavaScript/TypeScript and Dart expose the same operations with camelCase method names such as `listImages`, `deleteImage`, `pullImage`, `run`, `exec`, `logs`, `list`, `stop`, and `deleteContainer`.
