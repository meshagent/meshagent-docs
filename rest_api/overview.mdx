...
- **Project settings & integrations**: Update project settings, model routing configuration, API keys, and OAuth clients
...
meshagent service-account api-key create my-key --service-account default --activate
```
...
Room, agent, repository, and feed direct access is managed through IAM-style resource policies backed by OpenFGA.

| SDK method                                                     | HTTP route                                                                                  | What it does                                      |
| -------------------------------------------------------------- | ------------------------------------------------------------------------------------------- | ------------------------------------------------- |
| `get_resource_policy(project_id, resource_type, resource_id)`  | `GET /accounts/projects/{project_id}/iam/{resource_type}/{resource_id}/policy`              | List direct policy bindings for a resource.       |
| `grant_resource_policy(project_id, resource_type, resource_id, subject, roles)` | `POST /accounts/projects/{project_id}/iam/{resource_type}/{resource_id}/policy:grant` | Add or replace direct roles for a subject.        |
| `revoke_resource_policy(project_id, resource_type, resource_id, subject)` | `POST /accounts/projects/{project_id}/iam/{resource_type}/{resource_id}/policy:revoke` | Remove all direct roles for a subject/resource.   |
| `list_access_bindings(project_id, subject)`                    | `POST /accounts/projects/{project_id}/access:bindings`                                      | List direct resource bindings for a subject.      |
| `get_effective_access(project_id, subject, resource, relations?)` | `POST /accounts/projects/{project_id}/access:effective`                                  | Evaluate effective roles for a subject/resource.  |
| `test_access(project_id, subject, resource, relation)`         | `POST /accounts/projects/{project_id}/access:test`                                          | Test a single effective relation.                 |
...
API keys belong to service accounts and are managed under the owning service account.

| SDK method                                                        | HTTP route                                                                                      | What it does                              |
| ----------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- | ----------------------------------------- |
| `create_api_key(project_id, service_account_id, name, description)` | `POST /accounts/projects/{project_id}/service-accounts/{service_account_id}/api-keys`            | Issue a new API key (returns value once). |
| `list_api_keys(project_id, service_account_id)`                   | `GET /accounts/projects/{project_id}/service-accounts/{service_account_id}/api-keys`             | List keys (does not list key value).      |
| `delete_api_key(project_id, service_account_id, id)`              | `DELETE /accounts/projects/{project_id}/service-accounts/{service_account_id}/api-keys/{id}`     | Revoke a key.                             |
| `revoke_api_keys_by_msid(project_id, service_account_id, msid)`   | `POST /accounts/projects/{project_id}/service-accounts/{service_account_id}/api-keys:revoke`     | Revoke OAuth-session-created keys.        |
...
| `get_session_participant_counts(project_id, session_id)` | `GET /accounts/projects/{project_id}/sessions/{session_id}/participant-counts` | Participant counts. |
