...
If you need the concepts first, start with [Intro to Services](../overview).

## Project service or room service?

- **Project service**: available in every room in the project
- **Room service**: saved to one room

The same manifest can be deployed either way. Use `--global` for a project service or `--room myroom` for a room service.

Project service changes usually take effect when a room starts a new session. Room service changes can be reconciled during an active session.

If you only need a temporary run and do not want to save it as a deployed service, use the [Containers API](../../room_api/containers) instead.

## `Service` or `ServiceTemplate`?

- **`Service`**: ready to run as written
- **`ServiceTemplate`**: collects values first, then renders a `Service`

Use `Service` when the configuration is already known. Use `ServiceTemplate` when a user or installer should provide values such as prompts, API keys, names, labels, or URLs.

## `container` or `external`?

- **`container`**: MeshAgent pulls the image and runs it for you
- **`external`**: the service already runs somewhere else and MeshAgent routes traffic to it
