...
Both can be used for project services or room services. The difference is whether configuration is fixed or provided at runtime. For full details, see [Packaging Services](./packaging). For CLI deployment of templates (including validation), see [Deploying Services](./deployment).
