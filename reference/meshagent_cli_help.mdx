...
* `--key TEXT`: an api key to sign the token with
* `-rr, --room-rules TEXT`: a path to a rules file within the room that can be used to customize the agent&#x27;s behavior
* `--save-transcript / --no-save-transcript`: Save voice conversation transcript as a MeshDocument  [default: no-save-transcript]
* `-tpt, --transcript-path-template TEXT`: Path template for transcript MeshDocument. Supports &#123;participant_name&#125;, &#123;participant_id&#125;, &#123;agent_name&#125;, &#123;date&#125;, &#123;time&#125;  [default: transcripts/&#123;participant_name&#125;/&#123;date&#125;/&#123;time&#125;.transcript]
* `--help`: Show this message and exit.
