...
Building an agent with an LLM is easy. **Deploying it, scaling it, and integrating it into real applications is hard.** MeshAgent bridges that gap—it helps you **build new agents with ready-made templates, wrap existing agents from other frameworks, and get them working in production with the infrastructure that traditional frameworks leave as exercises for the developer**.

MeshAgent expands on traditional agent frameworks by focusing on **inter-framework interoperability, rich human-in-the-loop workflows, and seamless multi-device reach.** You can build agents directly with MeshAgent's templates, or wrap agents built with familiar frameworks—OpenAI Agents SDK, Semantic Kernel, CrewAI, LangChain, Pydantic AI. Either way, MeshAgent handles everything you need for production scale: 
...
MeshAgent organizes agents into two categories based on how you interact with them:

### Participant-Based Agents
Interactive agents you communicate with via voice or text. These agents:
- Appear as participants in the room (like chat members)
...
**Examples**: ChatBot, VoiceBot

**Use when**: You need a conversational interface, user assistance, or iterative problem-solving.

### Ambient Agents
Background agents that execute specific tasks and return results. These agents:
...
**Examples**: TaskRunner, Worker, Indexer

**Use when**: You need to process data, wrap existing agents from other frameworks, handle background jobs, or respond to events.
...
* **[TaskRunner](./taskrunner)** – Can be used to wrap an existing agent from other frameworks (Pydantic AI, LangChain, CrewAI, etc.) to work seamlessly with MeshAgent. Requires only input/output schemas and an ask() method—no refactoring of your agent logic needed. 
...
- Build a ChatBot – Full tutorial with tools and deployment
- Wrap an Existing Agent – Integrate agents from other frameworks