...
Variables define what values users can provide when a container-based service launches. You can restrict inputs or hide sensitive ones. Variables are substituted into container fields (most often `container.environment` values) using `{variable_name}` placeholders. 
