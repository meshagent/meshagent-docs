...
2. **Chatbot with Out-of-the-Box MeshAgent Tools**: Extend our chat agent by adding MeshAgent's prebuilt tools to interact with the user and write documents to the room.
3. **ChatBot with Out-of-the-Box MeshAgent Tools and Custom Tools**: Add our own tools to the agent for use-case specific tasks. 
...
Next in our main.py file let's create the basic ChatBot. We'll give it one rule / system instruction for now letting it know to reply to the user with a fun fact after it addresses their request. 
...
Now that we have the code for basic Chatbot let's try it out!
...
## Adding MeshAgent Tools to our Chatbot
Next, let's give our agent the ability to interact with the user, convert documents to markdown, and write documents to the room. We can do this by adding two required toolkits to our agent. Toolkits represent a group of tools that are used for a particular purpose. 
...
## Adding Custom Tools to our Chatbot
...
Once we're satisfied with how the agent is performing we can move on to deploy and share the agent. 
