...
```console
$ meshagent container build [OPTIONS]
```
...
* `--dir TEXT`: Directory to build from (defaults to current directory)
* `--git TEXT`: git url to build from
* `--ref TEXT`: git ref to build from
* `--cred TEXT`: Docker creds (username,password) or (registry,username,password)
* `--name TEXT`: [default: cli]
* `--role TEXT`: [default: user]
