...
- **[Sync API](./sync) `SyncClient`:** Offers document synchronization capabilities for shared room documents.
- **`ServicesClient` (Python/TypeScript/Dart):** List and restart room services. For deployment patterns, see [Services & Containers](../services_room_containers/overview).

Some of the guides in this section build on top of a specific Room API:

- document guides live under **Sync API**
- room-secret and OAuth guides live under **Secrets API**
- session behavior is covered in [Sessions](./sessions)
...
- connect to `myroom` from any SDK using the examples in [Quickstart](../introduction/cli_quickstart)
...
- Sub-clients are exposed as PascalCase properties: `room.Agents`, `room.Database`, `room.Developer`, `room.Livekit`, `room.Memory`, `room.Messaging`, `room.Queues`, `room.Secrets`, `room.Storage`, and `room.Sync`.
