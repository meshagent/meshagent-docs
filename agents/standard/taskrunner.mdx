...
- You want to expose an agent as a callable tool for other agents
- You want to run an agent in the background, not have a conversation with it
- You want a clean context window for each invocation. (To persist conversation state across runs, enable thread selection and provide a path so the runner can load and update the thread document)
...
### Step 1: Run a TaskRunner once (CLI)
...
### Step 2: Join a TaskRunner to a room (CLI)
...
<CodeGroup>
    ...
    --arguments '{"prompt":"Write a poem about ai agents", "tools":[{"name": "web_search"}],"model":null}'
    ```
</CodeGroup>
