...
Use `meshagent token generate` when you need to sign a participant token yourself.

This is useful for custom clients, CLI coding agents, or temporary room identities that need to access room APIs directly.
...
Tokens that will make LLM proxy requests must include the `api.llm` grant. Tokens without `api.llm` are rejected by the proxy.
...
- `api`: the [API scope](./api_scopes) for that participant. For LLM proxy access, this must include `llm`.
...
```bash
meshagent token generate --input token-spec.yaml --output room.token
```
...
- [Using CLI Coding Agents with the MeshAgent LLM Proxy](../agents/routing/cli_agents)
