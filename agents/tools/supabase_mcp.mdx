...
import MCPSupabaseYAML from "/snippets/examples/cli/supabase-mcp/toolkit.mdx"
...
- The [MeshAgent CLI](../../introduction/cli_quickstart) installed (either globally via `brew install meshagent` or in a python environment via `uv add "meshagent[all]"`)
...
When you create a room secret with `--for-identity supabase`, that secret belongs to the `supabase` identity — not to you. Running `meshagent room secret list --room myroom` while authenticated as yourself won't show it.
...
meshagent room secret delete --room myroom supabase
```
...
This command connects to the Supabase MCP server and exposes it as a toolkit in your room. The `{supabase}` placeholder in `--header-secret` resolves the room secret you created in Step 2 by secret ID or name, so in this example it matches the room secret stored as `supabase`.
...
meshagent room agent list-toolkits --room myroom
```
