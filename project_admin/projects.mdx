...
First run `meshagent setup` to sign in, or continue with the current account if you are already signed in, then choose or create a project and activate that project from the CLI. If the Codex CLI is installed, setup can also optionally reuse an existing MeshAgent Codex profile for the active project or create a new one after the project is activated. For the full command surface, see [MeshAgent CLI](../reference/meshagent_cli_help).
