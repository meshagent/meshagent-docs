...
var protocol = Helpers.CreateWebSocketProtocol("sample_user", "my-example-room");
...
room.Developer.On("log", data =>
{
  ...
  var logData = data.GetValueOrDefault("data");
  Console.WriteLine($"[DeveloperLog] {type}: {JsonSerializer.Serialize(logData)}");
});
...
```
```dotnet C#
await room.Developer.Log("warn", new Dictionary<string, object?>
{
  { "status", "Something might be wrong" }
});
```
...
```
```dotnet C#
room.Developer.On("log", data => Console.WriteLine($"Developer log event: {data}"));
```
</CodeGroup>
...
{
    Console.WriteLine($"Failed to enable developer logs: {err}");
}
```
</CodeGroup>
