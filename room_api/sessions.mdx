...
meshagent session show SESSION_ID
```

Use `meshagent session list` to find recent sessions in the active project, then use `meshagent session show SESSION_ID` to inspect the recorded events for one specific run.
...
Use [Observability](../observability/intro) when you want the broader telemetry model, and use the [REST API](../rest_api/overview) when you need to list or inspect sessions programmatically.
