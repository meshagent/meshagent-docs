...
> **CLI Note**: Room-scoped secrets and OAuth actions are under `meshagent room secret`. For project wide or image-pull secrets, use `meshagent secret`.
...
<CodeGroup>
```bash CLI
meshagent room secret delete --room=myroom --delegated-to=participant_123 SECRET_ID
```
```python Python
