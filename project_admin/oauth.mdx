...
For project-level LLM proxy access, include the `llm:invoke` scope. OAuth-authenticated requests to the MeshAgent OpenAI or Anthropic proxy also require `Meshagent-Project-Id: <project_id>` and a user who has the `can_use_llm_proxy` project permission.
