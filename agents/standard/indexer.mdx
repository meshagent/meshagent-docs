...
- An embedding provider (default requires the OpenAI Python SDK and `OPENAI_API_KEY`).
...
## Operational Notes
- Index tables live in the room database. You can explore them with `meshagent database` commands or `room.database.*` APIs.
- `StorageIndexer` debounces index rebuilds; large bursts consolidate into a single optimization pass.
- Deleting a file triggers removal of its rows automatically.
- For cross-room reuse, host indexers via `ServiceHost` or `RemoteAgentServer` just like other agents.
