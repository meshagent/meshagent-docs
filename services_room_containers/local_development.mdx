...
2. **Author a manifest**: Use ``kind: Service`` when deploying a project service or ``kind: ServiceTemplate`` for room services. 
3. **Push to a registry**: Make sure your image is available for MeshAgent to use as a service. 
...
- [Configure](./project_services/configuration) and [Deploy](./project_services/deployment) a Project Service to make the service available in every room.
- [Configure](./room_services/configuration) and [Deploy](./room_services/deployment) a Room Service to make the service available in a specific room.
