...
### Step 2: Install MeshAgent (Python)
From your terminal create or activate a virtual environment, for help with this see our [Machine Setup Guide](../reference/machine_setup).
...
meshagent project activate -i # Select a project to activate
# create an api key if necessary. The key value will be displayed only once.
meshagent api-key create --activate my-api-key
```
</CodeGroup>
...
These agents will be available in the Room while you're running them from the CLI. If you want the agents to persist in the room you will need to deploy them as a project or room service. 
