---
title: "MailBot (Archived)"
---

import CliMailbotYaml from "/snippets/examples/archive/cli/mailbot/meshagent.mdx"
import SimpleMailbot from "/snippets/examples/archive/python/deployable/mailbot/mailbot.mdx"
import ProvisionMailbox from "/snippets/examples/archive/python/deployable/mailbot/provision_mailbox.mdx"
import ScratchDockerfile from "/snippets/examples/python/deployable/Dockerfile.mdx"
import SDKMailbotYaml from "/snippets/examples/archive/python/deployable/mailbot/meshagent.mdx"

> **Archived:** The dedicated `meshagent mailbot` CLI runtime is no longer the recommended path for new docs. Use [`meshagent process`](../process/overview) with a `mail` channel for new mail-driven agents. This page remains as historical reference for the older MailBot-specific runtime.
...
- You need speech, use [VoiceBot](../standard/voicebot)
...
MailBots can be extended with rules, tools, and skills depending on the workflow they need to handle. See [Tools and Toolkits](../tools/tools_and_toolkits) and [Skills Overview](../../agent_skills/overview).
...
**Option 2: Deploy a YAML spec**
Create a `meshagent.yaml` that defines how to run the service. In this case we can create a `ServiceTemplate` that allows us to pass an email as the variable
...
```bash bash
meshagent service create-template --file meshagent.yaml --room quickstart --value email_address="$EMAIL_ADDRESS"
```
...
For full details on the service spec and deployment flow, see [Packaging and Deploying Services](../../services/deployment/deploy_services). If you want to deploy private MailBot skills with the service, see [Bundle Private Skills with a Custom Service](../../agent_skills/packaging_and_deploying).
...
| `skill_dirs`         | `list[str] \| None`           | Directories containing agent skills; added to the rules so the model can discover and use them. See [Skills Overview](../../agent_skills/overview). |
...
- **Skills integration:** `skill_dirs` append skill descriptions to rules for tool execution guidance. See [Skills Overview](../../agent_skills/overview).
...
- [VoiceBot](../standard/voicebot): Create a conversational speech/voice based agent

Learn how to deploy and run agents across rooms in a project or in specific rooms:
- [Services and Room Containers](../../services/intro): Understand agent deployment patterns
- [Skills Overview](../../agent_skills/overview): Learn when MailBots should use skills
