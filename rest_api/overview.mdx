...
Project settings are independent documents named `openai`, `anthropic`, `otel`, `admission`, `room`, and `room_roles`. The REST path for `room_roles` is `room-roles`; SDK clients translate that name automatically. A missing document is unconfigured and is not read from the legacy project settings field.
