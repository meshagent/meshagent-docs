...
meshagent rooms create toolsroom --if-not-exists
meshagent room connect --room=toolsroom --identity=math-agent -- python3 tools-adder.py
```
</CodeGroup>

This command runs the local Python agent and registers the `math-toolkit` toolkit into `toolsroom`.
...
While the agent is running, from a new tab in your terminal invoke the tool. The result will print to the terminal.
