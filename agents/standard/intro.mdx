...
- Tool access (use built-ins for web search, storage, image generation, etc.)
- MCP connectors 
...
- Define your own agent classes and add custom agent logic
- Create custom tools and schemas
- Integrate external agent frameworks 
- Define custom lifecycle hooks, routing logic, or other orchestration 
...
* **[ChatBot](./buildanddeploychatbot):** Conversational text agent. Manages a shared thread document, streams tool calls, and persists exchanges.
* **[VoiceBot](./buildanddeployvoicebot):** Conversational speech/voice agent. Integrates with LiveKit for audio in/out and supports the same tool interface as the ChatBot.
...
| [Project Service](../../services_room_containers/packaging_and_deploying/project_service_example)     | Deploy once to run automatically in every room of a project (admin only).  | Always-on shared agents            |
| [Room Service](../../services_room_containers/packaging_and_deploying/room_services_example)        | Deploy to a single Room only. Other Rooms in the Project won't see it.  | Agents that should stay room-scoped. |
...
- [Build a Chat Agent](./buildanddeploychatbot): Iteratively build up a chat-based agent, starting from the base ChatBot class, then adding built-in tools, and custom tools to your agent.
