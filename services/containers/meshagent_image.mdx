...
The tag can be a full configured registry reference like
`registry.meshagent.com/<project-key>/<repository>:<tag>`, or shorthand like
`<repository>:<tag>` and `<project-key>/<repository>:<tag>`. The CLI fills in
the configured registry from `/config`, and if you omit the project key it uses
...
- packed builds accept the same shorthand tag forms as `pack`
