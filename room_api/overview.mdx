...
Most Room APIs are also exposed as built-in room toolkits. That means agents can use the same room capabilities through tool calls as well as through the CLI and SDKs. In practice, this is how agents use built-in toolkits like `storage`, `database`, `memory`, `sync`, `queues`, `messaging`, `secrets`, `developer`, and `containers`.
