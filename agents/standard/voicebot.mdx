...
If your agent only handles text-based chat, use [ChatBot](./chatbot) instead. For non-interactive background agents, see [Worker](./worker) or [TaskRunner](./taskrunner). For email based agents use [MailBot](./mailbot).
...
Once your agent works locally to make it always available you'll need to [package and deploy](../../services_room_containers/packaging) it as a project or room service. You can do this using the CLI, by creating a YAML file, or from MeshAgent Studio.
...
- [MailBot](./mailbot): Email based agents.
