---
title: "TaskRunner (Archived CLI Runtime)"
---

import CliTaskRunnerYaml from "/snippets/examples/archive/cli/taskrunner/meshagent.mdx"
import LLMTaskRunnerService from "/snippets/examples/archive/python/deployable/taskrunner/llm_taskrunners_service.mdx"
import ScratchDockerfile from "/snippets/examples/python/deployable/Dockerfile.mdx"
import TaskRunnerYaml from "/snippets/examples/archive/python/deployable/taskrunner/meshagent.mdx"
import RunTaskRunner from "/snippets/examples/archive/python/agents/taskrunner/run_taskrunner.mdx"
import InvokeLLMTaskRunner from "/snippets/examples/archive/python/deployable/taskrunner/invoke_llm_taskrunner.mdx"

> **Archived:** The dedicated `meshagent task-runner` CLI runtime is no longer the recommended path for new docs. Use [`meshagent process`](../process/overview) with a `toolkit` channel when you want a process-backed agent to be callable like a tool. This page remains as historical reference for the older TaskRunner CLI runtime.
...
If you need a conversational agent, use [ChatBot](./chatbot). For real-time speech, use [VoiceBot](../standard/voicebot). For background queue work, use [Worker](./worker) and for email use [MailBot](./mailbot).
...
- **CLI** (`meshagent room agent invoke-tool` to invoke a task runner you've joined to the room using `meshagent task-runner join`)
...
TaskRunners can be extended with rules, tools, and skills depending on how much reusable workflow guidance they need. See [Tools and Toolkits](../tools/tools_and_toolkits) and [Skills Overview](../../agent_skills/overview).
...
```bash bash
meshagent room agent invoke-tool --room quickstart --toolkit mytaskrunner --tool run_mytaskrunner_task --arguments '{"prompt":"Draft a short product description for a bluetooth speaker", "tools":[], "model":null}'
```
...
```bash bash 
meshagent task-runner deploy --service-name mytaskrunner --room quickstart --agent-name mytaskrunner --web-search --storage --room-rules "agents/mytaskrunner/rules.txt" --rule "You are a helpful assistant"
```
...
```bash bash 
meshagent task-runner spec --service-name mytaskrunner --agent-name mytaskrunner --web-search --storage --room-rules "agents/mytaskrunner/rules.txt" --rule "You are a helpful assistant"
```
...
For full details on the service spec and deployment flow, see [Packaging and Deploying Services](../../services/deployment/deploy_services). If you want to ship private TaskRunner skills with the service, see [Bundle Private Skills with a Custom Service](../../agent_skills/packaging_and_deploying).
...
- [VoiceBot](../standard/voicebot) for voice-based agents
- [Worker](./worker) for background queue processing
- [Skills Overview](../../agent_skills/overview) for reusable workflows that complement schemas and tools
