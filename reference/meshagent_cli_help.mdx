...
* `[PROJECT_ID]`: Project id. If omitted, an interactive picker is shown in a TTY.
...
Show the activated API key for a project.
...
Deploy a packaged service into a room.
...
Run a packaged service container in a room.
...
Create a room in the project. Use this before meshagent deploy --room when the room does not already exist, then deploy with &#x27;meshagent deploy PATH --room &lt;room&gt; --tag &lt;tag&gt; --public --domain &lt;domain&gt;&#x27;.
...
```console
$ meshagent  update [OPTIONS] FEED_ID
```
...
```console
$ meshagent  show [OPTIONS] FEED_ID
```

**Arguments**:

* `FEED_ID`: Feed id to show  [required]
...
```console
$ meshagent  delete [OPTIONS] FEED_ID
```
...
```console
$ meshagent  send [OPTIONS] FEED_ID
```
...
```console
$ meshagent  send-batch [OPTIONS] FEED_ID
```
...
```console
$ meshagent  update [OPTIONS] SUBSCRIPTION_ID
```
...
```console
$ meshagent  show [OPTIONS] SUBSCRIPTION_ID
```

**Arguments**:

* `SUBSCRIPTION_ID`: Subscription id to show  [required]
...
```console
$ meshagent  delete [OPTIONS] SUBSCRIPTION_ID
```
...
* `--project-id TEXT`: A MeshAgent project id. If empty, the activated project will be used.  [default: (dynamic)]
* `-d, --domain TEXT`: Domain name to route (unique per project). Keep it short and DNS-safe; long room-name-derived domains may be rejected.  [required]
* `--room TEXT`: Room name
...
* `--project-id TEXT`: A MeshAgent project id. If empty, the activated project will be used.
* `--room TEXT`: Existing room name. meshagent deploy does not create rooms; create the room first with &#x27;meshagent rooms create --name &lt;room&gt;&#x27;.
...
Create or update a room service from an image, optionally building it first. The target room must already exist. Create it first with &#x27;meshagent rooms create --name &lt;room&gt; --if-not-exists&#x27;. Happy path for a Dockerfile app: create the room with &#x27;meshagent rooms create --name &lt;room&gt; --if-not-exists&#x27;, then run &#x27;meshagent deploy PATH --room &lt;room&gt; --tag &lt;tag&gt; --public --domain &lt;domain&gt;&#x27; so deploy creates the service and the public route together. If PATH does not include a Dockerfile yet, create a minimal Dockerfile in the app directory first or create one elsewhere in PATH and pass it with --dockerfile-path.
...
* `--domain TEXT`: Create or update a room route for the deployed service and return a public URL. Use this with --public when you need an external URL from deploy. In MeshAgent dev/.life, domains must end with .meshagent.dev; in prod they must end with .meshagent.app. Requires exactly one published service port.
...
* `-n, --annotations TEXT`: annotations in json format &#123;&quot;name&quot;:&quot;value&quot;&#125;
* `--storage-write-path TEXT`: Optional room storage folder to write scheduled payloads into
* `--help`: Show this message and exit.
...
* `-n, --annotations TEXT`: annotations in json format &#123;&quot;name&quot;:&quot;value&quot;&#125;
* `--storage-write-path TEXT`: Optional room storage folder to write scheduled payloads into
* `--clear-storage-write-path`: Clear the stored room storage folder for this task
* `--help`: Show this message and exit.
...
* `--use-memory TEXT`: Use memories toolkit for &lt;name&gt; or &lt;namespace&gt;/&lt;name&gt;
* `--memory-model TEXT`: Model name for memory LLM ingestion
* `--dataset-namespace TEXT`: Dataset namespace (e.g. foo::bar)
* `--computer-use`: Enable computer use
* `--starting-url TEXT`: Initial URL to open when starting a computer-use browser session
* `--allow-goto-url`: Expose the goto URL helper tool for computer use
* `--document-authoring`: Enable MeshDocument authoring
...
* `tables`: List dataset tables in a room.
...
* `--project-id TEXT`: A MeshAgent project id. If empty, the activated project will be used.  [default: (dynamic)]
* `--room TEXT`: Room name  [default: (dynamic)]
* `-q, --query TEXT`: SQL statement to execute  [required]
* `-t, --table TEXT`: Table name to register in SQL context (repeatable)
* `-n, --namespace TEXT`: Namespace path segments (repeatable). Example: -n prod -n analytics
* `--tables-json TEXT`: JSON array of table refs
* `--tables-file TEXT`: Path/URL to JSON array of table refs
* `--params-json TEXT`: JSON object of SQL parameters, sent as one Arrow parameter row
* `--params-file TEXT`: Path/URL to JSON object of SQL parameters
* `--branch TEXT`: Dataset branch name (defaults to main)
* `-v, --version INTEGER`: Historical table version to read (defaults to latest on the branch)
* `--pretty / --no-pretty`: Pretty-print JSON  [default: pretty]
...
Connect to a room and run a local command with MESHAGENT_API_URL, MESHAGENT_PROJECT_ID, MESHAGENT_TOKEN, OPENAI_API_KEY, ANTHROPIC_API_KEY, and MESHAGENT_ROOM set. Use -- before the local command.
...
```console
$ meshagent  logger [OPTIONS] COMMAND [ARGS]...
```
...
```console
$ meshagent  logger create [OPTIONS]
```
...
```console
$ meshagent  logger update [OPTIONS] LOGGER_ID
```
...
```console
$ meshagent  logger show [OPTIONS] LOGGER_ID
```

**Arguments**:

* `LOGGER_ID`: LLM logger id to show  [required]
...
```console
$ meshagent  logger delete [OPTIONS] LOGGER_ID
```
