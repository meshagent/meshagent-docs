...
### Anthropic
Base URL for raw HTTP requests: `{room_url}/anthropic/v1`.

When you use the MeshAgent Anthropic SDK helper or the injected `ANTHROPIC_BASE_URL`, the base URL is `{room_url}/anthropic`; the Anthropic SDK appends the `/v1` paths itself.
...
For OpenAI compatible SDKs, you can also set `OPENAI_BASE_URL` to the room proxy and pass the MeshAgent room token as `api_key`. Room containers set `OPENAI_BASE_URL`, `ANTHROPIC_BASE_URL`, and `MESHAGENT_SESSION_ID` automatically.
...
## Related topics
- [Terminal-Based Coding Agents](./terminal_based_coding_agents)
