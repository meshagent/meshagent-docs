...
5. The chatbot invokes the LLM, streams back tokens, and appends responses to the thread document.
6. Toolkits or reasoning tools can be used mid-conversation when invoked by the LLM.
...
| `async def get_thread_toolkits(thread_context, participant)`   | Resolves all toolkits available to a given participant, including built-in reasoning tools. |
...
This subclass wires the chatbot to the OpenAI Responses API and applies a a system prompt. You can call the chatbot into a room directly by running `meshagent service run "main.py" --room=gettingstarted` from your terminal. Next from the [MeshAgent Studio](https://studio.meshagent.com) you can go to the **Sessions** tab, open the **gettingstarted** room, and begin chatting with your agent. 

To extend the Chatbot with Tools and learn how to deploy a Chatbot see the next article on [building and deploying a MeshAgent chatbot](./buildanddeploychatbot).
...
To continue learning or put your chatbot into action:

- [Build and Deploy a ChatBot](./buildanddeploychatbot): Step-by-step guide to configuring adapters, adding toolkits, and running your chatbot in a MeshAgent room.
...
- [MailWorker](./mailworker): Integrates with messaging or email systems.
