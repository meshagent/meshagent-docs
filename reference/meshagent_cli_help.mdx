...
```console
$ meshagent api-key show [OPTIONS]
```
...
* `--focus TEXT`: Project focus for non-interactive use. Use stable IDs: webserver (Web App), backend-agent (Agent Toolkit), chatbot (OpenAI Chatbot), chatbot-anthropic (Anthropic Chatbot), chatbot-ui (Agent UI), or contact-form (Contact Form).
...
* `get`: Show a managed agent configuration.
...
Show a managed agent configuration.
...
```console
$ meshagent feed show [OPTIONS] FEED_ID
```
...
```console
$ meshagent subscription show [OPTIONS] SUBSCRIPTION_ID
```
...
```console
$ meshagent registry show [OPTIONS] REPOSITORY_ID
```
...
* `--extra-port TEXT`: Add an extra route path to DOMAIN as PORT:/path. Can be passed multiple times. The port must already be published by a room service.
...
```console
$ meshagent llm logger show [OPTIONS] LOGGER_ID
```
