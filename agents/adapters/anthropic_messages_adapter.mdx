...
## Key features
- **Model defaults:** Reads the model name from the constructor (`model=`) or the `ANTHROPIC_MODEL` environment variable. Override per call by passing `model` to `create_response()`.
...
- `client` - reuse an existing Anthropic client; otherwise the adapter builds one via `meshagent.anthropic.proxy.get_client`.
- `base_url` - override the provider base URL used when the adapter creates its own client. Defaults to `ANTHROPIC_BASE_URL` when omitted.
...
- [MeshAgent LLM Proxy](../routing/llm_proxy): How requests are routed and metered inside rooms
...
- [Process Agents Overview](../process/overview): Shows where adapters are used in the recommended runtime
