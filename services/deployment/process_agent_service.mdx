...
The chat-facing annotations and `--thread-dir ".threads/news-agent"` settings keep the chat UI thread-aware while still letting mail and queue work route into the same agent. In the service YAML, include both `meshagent.chatbot.thread-dir` and `meshagent.chatbot.thread-list` with `dataset://` paths. That matches the output from `meshagent process spec` and makes the thread list location explicit for Studio and other chat clients.

### Participant token and permissions

The [`environment`](./deploy_services#container-environment) section injects a `MESHAGENT_TOKEN` with the API scopes this service needs for messaging, storage, queue handling, and agent/tool registration.
