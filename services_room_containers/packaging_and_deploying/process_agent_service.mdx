---
title: "Deploy a Process Agent Service"
---
...
### One process agent, several entry points
...
Instead of coordinating a separate ChatBot, MailBot, and Worker, the same process agent handles all three flows.
...
- the same process agent receives that queued message through `--channel=queue:news-updates`
...
Because this is one process agent, those flows all share the same identity, tools, and thread model.
...
- [Process Overview](../../agents/process/overview)
- [Using MeshAgent Process](../../agents/process/using_process)
