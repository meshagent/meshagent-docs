...
- The UI (or calling service) requests the builder list via `get_thread_tool_providers`, lets the user pick tools, then sends a `tools` array where each entry matches a builder `name` and config schema.
...
1. **Discover providers**: the client sends `get_thread_tool_providers`; the chatbot replies with the builders it supports.  
...
- **Granular capabilities**: enable heavier tools—like image generation or shell access—only when the user opts in for that specific message.  
- **User-selected connectors**: let people opt into MCP servers, web search, or proprietary APIs on a per-conversation or per-message basis.  
- **Parity with consumer chat UIs**: deliver a ChatGPT-style experience where tools appear as checkboxes the user can toggle.  
- **Operational safety**: avoid instantiating heavy or privileged toolkits unless they are explicitly requested.

Static and dynamic patterns can be mixed freely—keep always-on defenses in the constructor, then layer builder-driven toolkits on top for anything optional or contextual.
