...
**Both options below deploy the same MailWorker - choose based on your workflow:**
...
You can also use the MeshAgent SDK to manage mailboxes and create an email agent by extending the `MailWorker` class. For most use cases the CLI is sufficient and a faster way to get started, while the SDK allows you further customization. 
...
Send an email to the mailbox and wait for it's response, you can verify the `.emails/` folder and logs in [MeshAgent Studio](https://studio.meshagent.com).
