...
**Add authentication** — If your MCP server requires OAuth, add an `oauth` configuration to the `mcp` endpoint. See the [`oauth` fields](./service_manifest_reference#ports-endpoints) in the field reference.
...
- [MCP endpoint fields](./service_manifest_reference#ports-endpoints) — Full field reference for MCP endpoint configuration.
