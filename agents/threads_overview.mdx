...
In MeshAgent, a thread is addressed by a thread path and stored by the agent's configured thread storage backend. By default, process agents store thread history in the room dataset. Meshdocument storage is still supported for older or explicitly configured flows. A thread is the durable state behind things like:
