...
- Your agent processes email, instead use [MailWorker](./mailworker)
...
If you've turned on any tools or added the `--room-rules` flag to your agent you can toggle tools on and off and modify the agents `rules.txt` file to refine the agent's behavior. Changes to the tools and `rules.txt` will be applied per message. 
...
> **Note:** If you previously deployed the CLI ChatBot with name "chatbot" you will need to give your SDK based chatbot a unique name. 
...
- [MailWorker](./mailworker): Interact with an agent via email.
