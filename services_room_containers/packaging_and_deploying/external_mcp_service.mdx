...
**Add authentication** — If your MCP server requires OAuth, add an `oauth` configuration to the `mcp` endpoint. See the [`ports.endpoints`](./packaging#ports-endpoints) section in the field reference.
...
- [MCP endpoint fields](./packaging#ports-endpoints) — Full field reference for MCP endpoint configuration.
