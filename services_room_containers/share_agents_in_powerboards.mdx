...
container:
  ...
  command: "/usr/bin/meshagent/meshagent chatbot service --agent-name=spanish --image-generation=gpt-image-1 --require-read-only-storage --model=gpt-5.2 --rule='speak spanish' -rr=/agents/spanish/rules.txt"
  storage: 
external: 
```
