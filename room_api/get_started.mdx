...
4. Connect to your first MeshAgent Room


## Creating an Account, Project, and MeshAgent API Keys

1. Go to [studio.meshagent.com](https://studio.meshagent.com/) and sign in or sign up for an account.
2. Create a project and give it a name (e.g., MyFirstAgentProject)
3. Navigate to the API Keys tab and click + New API Key. Give your key a name and description then click "ok". Next, click on the key to copy the MESHAGENT_KEY_ID, MESHAGENT_PROJECT_ID, and MESHAGENT_SECRET. You will need to export these as environment variables to connect to your Project.
...
- **Project**: Groups related Rooms and the Agents that serve them. It is best to create a different Project for each use case you are working on.
- **Room**: A dedicated space for humans and Agents to collaborate. 
- **Agent**: ChatBots, VoiceBots, TaskRunners, and Workers that you build and deploy with custom tools or out of the box MeshAgent tools. Agents live inside a Project and once deployed are automatically available in every room under that project. 
