...
By default, `meshagent ask` includes a concise pre-tool preamble rule. Disable it when you want the model to run without that default instruction:

```bash bash
meshagent ask \
  --message "Run this prompt with only my configured rules." \
  --no-preamble-rule
```
