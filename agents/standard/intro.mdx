...
MeshAgent ships several **built-in agent classes that you can use out of the box or extend with your own rules and tools**. Use them for: 
- **Rapid prototyping:** Spin up a ChatBot or VoiceBot in minutes without writing plumbing code
- **Production foundation:** Subclass or configure MeshAgent Agents with your own rules and tools to create specialized agents tailored to your use case. 
- **Framework integration**: Use a MeshAgent TaskRunner to integrate with agents from other popular libraries like LangChain, Semantic Kernel, CrewAI, PydanticAI, and more without re-architecting your agent. 
...
- Appear as participants in the room (just like human team members)
...
* **[ChatBot](./buildanddeploychatbot):** Conversational text-based agent. Manages a shared thread document, streams tool calls, and persists exchanges.
* **[VoiceBot](./buildanddeployvoicebot):** Conversational agent with real-time speech. Integrates with LiveKit for audio in/out and supports the same tool interface as the ChatBot.

#### Background Agents
Background (ambient) agents that execute specific tasks and return results. Best when you need to process data, wrap existing agents from other frameworks, handle background jobs, or respond to events. These agents:
...
## Next Steps
- [Agents Quickstart](./quickstart): Create your first ChatBot in 5 minutes
- [Build and Deploy a ChatBot](./buildanddeploychatbot): Full tutorial with tools and deployment
