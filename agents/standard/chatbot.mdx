...
5. ChatBot resolves this turn’s toolkits: it collects `ToolkitBuilder`s from `get_thread_toolkit_builders(...)`, merges per-turn `ToolkitConfig`s (from MeshAgent Studio/CLI/App UI), calls `make_tools(...)` to create any dynamic toolkits, and combines the result with any static toolkits.
