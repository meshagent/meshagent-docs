...
> Prerequisite: To use ``MailWorkers`` you must either be an admin of the MeshAgent project you are using or have the admin create and allocate an email address for the agent to use. Only admins can create and allocate email addresses for agents in a project. 
...
Set env vars, pick a queue, and choose a unique email address. If you’re self-hosting, you can override the default mail domain.
You'll also need to export the MeshAgent Project ID you can run ``meshagent project list`` from the CLI to get the IDs associated with the MeshAgent projects you have access to. 

```bash
export EMAIL_QUEUE="test_email"
export EMAIL_ADDRESS="myemail@mail.meshagent.com" # come up with a unique email
export MESHAGENT_PROJECT_ID="<your-project-id>"
# If self-hosting, you can also set:
# export MESHAGENT_MAIL_DOMAIN="mail.meshagent.com" # this would be your custom domain instead of the default mail.meshagent.com
```
