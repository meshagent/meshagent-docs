...
```bash bash 
meshagent room agent invoke-tool --room=toolsroom --tool=add --require-toolkit=math-toolkit --arguments='{"a": 5, "b":7}'
```
...
- Learn how to use custom tools inside a [Chat Agent](../standard/chatbot) or [Voice Agent](../standard/voicebot)
