...
Room services are scoped to a specific room. Typically services that require customization by room like allowing users to pass a custom system prompt to an agent or bringing an API key should be run as a room service. 
