...
import CliChatBotYaml from "/snippets/examples/archive/cli/chatbot/meshagent.mdx"
import Chatbot from "/snippets/examples/archive/python/deployable/mesh_tools_chatbot/mesh_tools_chatbot.mdx"
import ScratchDockerfile from "/snippets/examples/python/deployable/Dockerfile.mdx"
import ChatbotYaml from "/snippets/examples/archive/python/deployable/mesh_tools_chatbot/meshagent.mdx"

> **Archived:** The dedicated `meshagent chatbot` CLI runtime is no longer the recommended path for new docs. Use [`meshagent process`](../process/overview) for new process-based agents. This page remains as historical reference for the older ChatBot-specific runtime.
...
- Your agent needs voice/speech interaction, instead use [VoiceBot](../standard/voicebot)
...
meshagent chatbot join --room quickstart --agent-name chatbot --web-search --storage --room-rules "agents/chatbot/rules.txt" --rule "You are a helpful assistant"
```

When you add the `--room-rules "agents/chatbot/rules.txt"` flag and supply a file path for the rules, the file path is resolved relative to room storage. Create the file through a service template or room storage before relying on it. 
...
ChatBots can be extended with rules, tools, and skills depending on the kind of help you want them to provide. See [Tools and Toolkits](../tools/tools_and_toolkits) and [Skills Overview](../../agent_skills/overview).
...
Once your agent works locally to make it always available you'll need to [package and deploy](../../services/deployment/deploy_services) it as a project or room service. You can do this using the CLI, by creating a YAML file, or from MeshAgent Studio.
...
```bash bash 
meshagent chatbot deploy --service-name chatbot --room quickstart --agent-name chatbot --web-search --storage --room-rules "agents/chatbot/rules.txt" --rule "You are a helpful assistant"
```
...
```bash bash 
meshagent chatbot spec --service-name chatbot --agent-name chatbot --web-search --storage --room-rules "agents/chatbot/rules.txt" --rule "You are a helpful assistant"
```
...
As an alternative to the ``service run`` command you can also run the service locally by setting a the `port` parameter on `ServiceHost` and running ``python main.py`` in one tab in your terminal, then from another tab you can call individual agents into the room using the ``meshagent room agents call agent`` command. For example: 

```bash bash
meshagent setup # authenticate to meshagent
meshagent room agents call agent --url=http://localhost:8081/agent --room=quickstart --participant-name=chatbot
```
...
For full details on the service spec and deployment flow, see [Packaging and Deploying Services](../../services/deployment/deploy_services). If you want to ship reusable private skills with the chatbot, see [Bundle Private Skills with a Custom Service](../../agent_skills/packaging_and_deploying).
...
2. **Single Image**: Bundle your code and all dependencies into one image. This is good when you need to install additional libraries, but can result in larger images and slower pulls. If you build your own images we recommend [optimizing them with eStargz](../../services/containers/optimizing_containers).
...
| `llm_adapter`        | `LLMAdapter`                  |  Required. Adapter that talks to your model provider (for example `OpenAIResponsesAdapter`). It supplies session contexts and translates responses into MeshAgent events.                                      |
...
| `rules`              | `list[str] \| None`           | List of system or behavior rules appended to each session context.                                                 |
...
3. A session context (``AgentSessionContext``) is initialized using the ``llm_adapter``.
4. The message history is synchronized with the room’s ``"thread"`` document.
5. ChatBot resolves this turn’s toolkits by combining the toolkits required by `requires` with any always-on toolkits returned by `get_thread_toolkits(...)`.
6. The LLM is invoked with the prepared session context and the resolved toolkits; tool calls are routed back into those toolkits, tokens are streamed, and the results are appended to the thread document.
...
- **Context building:** `init_session()` asks the LLM adapter for a fresh context and appends any rules you provided. Override this if you need to preload the context with extra data.
...
- **Always-on toolkit model:** Tool availability is now driven by the toolkits you attach directly to the bot, plus any toolkits installed through `requires`.
- **Thread helpers:** Use `open_thread()` and `close_thread()` to manage `ThreadAdapter` instances, and call `ThreadAdapter.append_messages(...)` to replay stored messages into an existing session context (for example, when resuming a conversation).
...
| Method                                                         | Description                                                                                 |
| -------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| `async def init_session()`                                     | Creates a new session context using the configured `llm_adapter` and applies rules.            |
...
| `ThreadAdapter.append_messages(thread, session_context)`          | Loads historical messages into the current session context from the thread document.           |
| `async def prepare_llm_context(thread_context)`                | Hook for modifying the session context before invoking the LLM. Override as needed.            |

### Built-in Toolkits and Behaviors
``ChatBot`` automatically provides and manages several toolkits and utilities:
- **Reasoning Toolkit:** Allows structured “chain-of-thought” reasoning within the session context.
...
- [VoiceBot](../standard/voicebot): Adds streaming audio input/output for speech interactions.
...
- [Skills Overview](../../agent_skills/overview): Add reusable workflow guidance without turning everything into inline rules.

To learn more about deploying agents with MeshAgent
- [Packaging and Deploying Services](../../services/deployment/deploy_services): Package and deploy agents as room or project services.
- [Secrets and Credentials](../../secrets/overview): Learn how to store credentials securely for deployment
