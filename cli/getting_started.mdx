...
## Overview
This guide covers common CLI workflows for day-to-day MeshAgent development. If you haven't set up MeshAgent yet, start with the [Getting Started Guide](../introduction/get_started) first.
...
2. **Install the MeshAgent CLI**: You can install the CLI globally on your system or in a virtual environment. 
    **To install the CLI globally:**
    ...
    # Alternatively:
    Invoke-WebRequest -Uri "https://storage.googleapis.com/meshagent-cli-builds/0.16.0/meshagent.0.16.0.nupkg" -OutFile "C:\Downloads\meshagent.0.16.0.nupkg"
    choco install meshagent --source "C:\Downloads\meshagent.0.16.0.nupkg"
    ```
    ```bash Other
...
The CLI based agents expose many tools and functionality already, we recommend using these for most use cases. When you need additional custom functionality you can extend the base agent classes, for example the `ChatBot`. To run this locally and deploy it, expose your agent or tool via `ServiceHost` (local webhook server). A request from MeshAgent spins up your agent or tool, connects it to the Room, and manages lifecycle for the session. For example: 

<CodeGroup>
    <SimpleChatbot />
</CodeGroup>

To run the service locally use the `meshagent service run` command. 
...
Open [MeshAgent Studio](https://studio.meshagent.com), join `testroom`, and start chatting with your agent. You can iterate on the code and restart the service to test your changes.
...
- [Build a Chat Agent](../agents/standard/buildanddeploychatbot): Tutorial on creating a custom chatbot
