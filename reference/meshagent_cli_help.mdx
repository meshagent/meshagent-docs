...
* `port`: Manage mailboxes for your project
* `multi`: connect agents and tools to a room for...
...
* `--project-id TEXT`: A MeshAgent project id. If empty, the activated project will be used.  [required]
* `-r, --room TEXT`: Room name to route inbound mail into  [required]
* `-c, --container-id TEXT`: The container which should have it&#x27;s port forwarded  [required]
* `-p, --port TEXT`: The port to foward and the port to expose local:remote  [required]
* `--help`: Show this message and exit.

## `meshagent multi`

connect agents and tools to a room for testing or host them as a mesh service
