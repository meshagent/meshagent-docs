...
### `create(path, json=None, schema=None)`
...
```
```python Python
await room.sync.create(path="my/path", json={"key": "value"}, schema=my_schema)
...
- **Parameters**:
  ...
  - `schema`: Optional `MeshSchema` for validating and describing the document shape. The Python helper exposes this parameter; other SDKs can pass a schema when opening a document.
- **Description**: Create a new `MeshDocument` at the given path. The CLI create command requires `--schema`; Python SDK callers can pass a `MeshSchema` when creating a document.

### `describe(path, create=True)`
...
- **Parameters**:
  - `path`: Document path.
  - `create`: Accepted by the Python helper for compatibility; the current implementation ignores it and only describes the existing document.
...
### `open(path, create=True, initial_json=None, schema=None)`

<CodeGroup>
```python Python
doc = await room.sync.open(
  path="my/path",
  create=True,
  initial_json={"key": "value"},
  schema=my_schema,
)
...
- **Parameters**:
  ...
  - `initial_json`: Optional initial document JSON when creating a document through `open`.
  - `schema`: Optional `MeshSchema` to use when creating/opening the document.
