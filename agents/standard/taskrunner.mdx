...
Once you call the agent into a room, your TaskRunner becomes an invokable participant. You can discover and run it directly from the [MeshAgent Studio](https://studio.meshagent.com) via the **"Run Task..."** room menu option, from the CLI ``meshagent agents ask``, or programatically via ``room.agents.ask(...)`` in your code. 
...
> This example is intentionally minimal. In real projects, you’ll typically subclass TaskRunner to integrate an LLM, framework-based agent, or other custom business logic. See [TaskRunner examples](./taskrunner_examples) for more. 

## Next Steps

For examples of TaskRunners in Action see: 
- [TaskRunner Examples](./taskrunner_examples) demonstrates how to use an agent from another framework with MeshAgent, how to use TaskRunners as tools, how to use pre-built MeshAgent TaskRunners, and how to create a custom LLM TaskRunner from scratch. 
