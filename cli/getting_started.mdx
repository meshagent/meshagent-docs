...
2. **Install the MeshAgent CLI**: You can install the CLI globally on your system or in a virtual environment. 
    **To install the CLI globally:**
    ...
    # Alternatively:
    Invoke-WebRequest -Uri "https://storage.googleapis.com/meshagent-cli-builds/0.31.0/meshagent.0.31.0.nupkg" -OutFile "C:\Downloads\meshagent.0.31.0.nupkg"
    choco install meshagent --source "C:\Downloads\meshagent.0.31.0.nupkg"
    ```
    ```bash Other
...
If one agent should accept work from more than chat, you can also test a [channel-driven agent built with `meshagent process`](../agents/process/overview) locally:
...
> **Note:** `chatbot join` / `voicebot join` are the standard runtime when the agent joins the room directly, both locally and in MeshAgent-native container deployments. Use `chatbot service` / `voicebot service` only when you want the process to expose an HTTP endpoint and wire it through `ports:` in the service YAML.
...
| `meshagent room agent list-toolkits` | List toolkits (including TaskRunner tool wrappers) available in a room |
...
- [Packaging and Deploying Services](../services_room_containers/packaging_and_deploying/packaging): Learn about meshagent.yaml configuration
- [Packaging and Deploying Services](../services_room_containers/packaging_and_deploying/packaging): Learn how to deploy project and room services
