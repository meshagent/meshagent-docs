...
### Step 1: Create a `ChatBot` agent using ``ServiceHost`` 
This example shows a `ChatBot` with a custom rule to guide the agent's behavior and access to built-in MeshAgent Tools for web search and UI toolkits. For an agent this simple the CLI ChatBot would be sufficient. We're showing the SDK code here to demonstrate how to get similar functionality from code. 
...
To deploy your SDK ChatBot permanently, you'll need:
...
For your own custom agents, create a Dockerfile, build with `docker buildx`, and push to your registry with `--platform linux/amd64`.
