---
title: "Indexer (Archived Pattern)"
---

import StorageRagService from "/snippets/examples/archive/python/deployable/storage_rag/indexer.mdx"
import ScratchDockerfile from "/snippets/examples/python/deployable/Dockerfile.mdx"
import IndexerYaml from "/snippets/examples/archive/python/deployable/storage_rag/meshagent.mdx"

> **Archived:** This page documents an older indexing pattern that is still useful as reference for existing implementations. It is not part of the main `meshagent process` runtime path for new agent docs.

Indexers keep a room’s vector dataset fresh so other agents can run retrieval-augmented generation (RAG) queries. They watch for new content, chunk and embed it, then store both the raw text and embeddings in a table that `RagToolkit` powered agents can search.
...
5. **Persist**: upsert rows into a room dataset table and build full-text / vector indexes.
...
- writes rows into the configured dataset table (defaults to `storage_index`)
...
| `table` | `str` | Dataset table used to store rows and embeddings. |
...
For full details on the service spec and deployment flow, see [Service YAML](../../services/deployment/deploy_services).
...
- [Process Agents Overview](../process/overview) for conversation-based process agents
- [Voicebot](../standard/voicebot) for voice based agents
- [Process Agents Overview](../process/overview) for chat, mail, queue, and toolkit entry points
- [TaskRunner integrations](./taskrunner_agents_and_tools) and process queue channels for background and queue-based agents
