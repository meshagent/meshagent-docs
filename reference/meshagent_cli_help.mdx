...
* `--deploy`: Create or update a room service that references the built image. When a local packed Dockerfile exposes ports, they are added as published service ports.
* `--domain TEXT`: Create or update a room route for the deployed service. Requires --deploy and exactly one published service port.
