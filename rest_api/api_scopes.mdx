...
Repository patterns can be exact names or prefixes ending in `*`. If `registry` is absent, registry checks allow repositories covered by the container grant. If `registry.list` is absent but `pull`, `run`, or `write` are present, list access is inferred from those repository allowlists.
