...
If your service also needs reusable skills, use the same service spec structure and add skill mounts plus repeated `--skill-dir` flags in the container command. See [Packaging and Deploying Skills](../../skills/packaging_and_deploying).
...
| Key | Description |
|-----|-------------|
| `meshagent.secret.*` | Tells Powerboards to store this variable's value as a room secret. Powerboards prompts the user and creates the secret automatically. The CLI ignores these annotations — create the secret manually with `meshagent room secret set`. |
