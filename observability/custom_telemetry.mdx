...
description: Add your own OpenTelemetry spans, logs, and metrics inside a MeshAgent service or room-connected toolkit.
---
...
```bash bash
meshagent rooms create otel --if-not-exists
meshagent room connect --room=otel --identity=weather-toolkit -- python3 observability.py
```
