...
Each skill lives in its own directory. The only required file is `SKILL.md`.
...
- [Skills Overview](./overview): understand how skills fit with tools, toolkits, rules, and prompts.
- [Using Skills with Agents](./using_with_agents): load one or more skills with `--skill-dir`.
- [Packaging and Deploying Skills](./packaging_and_deploying): ship skills inside a MeshAgent service.
