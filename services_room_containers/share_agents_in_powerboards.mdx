...
Check out the [optimizing containers](./containers/optimizing_containers) documentation to learn more about improving agent/service start time for custom images. 
...
container:
  ...
  command: "/usr/bin/meshagent/meshagent chatbot service --agent-name=spanish --image-generation=gpt-image-1.5 --require-read-only-storage --model=gpt-5.2 --rule='speak spanish' -rr=/agents/spanish/rules.txt"
  storage: 
external: 
```
