...
2. A container that can run your code:
   - **Single image:** you build an image that includes your runtime deps and your code (recommended to [optimize with eStargz](../../services_room_containers/containers/optimizing_containers) for faster pulls).
   - **Two-image setup:** use a MeshAgent runtime image (like `python-sdk-slim`) and mount a tiny code-only image into it for faster startup and smaller pulls.

For this example, we use a standard MeshAgent runtime image and mount a tiny code-only image. The default YAML points at the public `python-docs-examples` image so you can still run the documentation examples without building your own images. If you want to build and push your own code image, follow the steps below and update the image mount section of the `meshagent.yaml` file.

**Build a code-only image (optional)**
1. Put your deployable examples under a `src/` folder (for example: `src/mesh_tools_chatbot/mesh_tools_chatbot.py`).
2. Create a scratch Dockerfile:
```dockerfile Dockerfile
FROM scratch
COPY ./src /src
```
3. Build and push with `docker buildx`:
```bash bash
docker buildx build . \
