...
See [Room Secrets](../secrets/room_secrets), [OAuth With Room Secrets](../secrets/oauth_flows), [API Scopes](../rest_api/api_scopes), and [Service YAML](../services/deployment/deploy_services).
...
### `exists(secret_id)`
- **Description**: Check whether a stored secret exists without retrieving the secret data.
- **Returns**: `bool`.
...
```
```python Python
if await room.secrets.exists(secret_id="my-secret"):
    print("Secret exists")

```
```typescript TypeScript
const exists = await room.secrets.exists({ secretId: "my-secret" });

```
```dotnet C#
var exists = await room.Secrets.Exists(secretId: "my-secret");
