...
The `LLMAdapter` is the base class that standardizes how MeshAgent talks to any language model provider (OpenAI, Anthropic, self-hosted, etc.). You pick or implement an adapter for your provider and can reuse it with any agent that accepts an `llm_adapter`. 

## Why it exists
Providers differ on request formats, tool-call protocols, streaming, system-role conventions, termination signals, etc. The adapter hides those differences so your agent logic stays provider-agnostic and portable.

## What an adapter does (conceptually)
The ``LLMAdapter`` supplies the methods your implementation uses to run a full model “turn.” (For a concrete reference, see the [OpenAI Responses Adapter](./openai_responses_adapter))
...
- `tool_providers(model: str)`: Optionally advertises `ToolkitBuilder` instances this adapter can supply for a given model. This is one way to to bundle built-in tools such as image generation or web search for a provider. Alternatively, agents can ignore this and manage the builders themselves. 
