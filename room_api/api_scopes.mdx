...
- `ApiScope.agent_default()` – enables Livekit, Queues, Messaging, Database, Sync, Storage, Containers, Developer, Agents, and Secrets access.
- `ApiScope.full()` – everything in `agent_default()` plus the Admin grant.

Use these helpers when you want broad access, then override individual fields when you need to lock things down.

## Scope fields
Each field is optional. When a field is `None` the runtime treats it as "no restriction" for that capability. When populated, the nested grant limits behaviour as described below.
...
- `build` and `logs`: booleans toggling build and log streaming support.
...
Combine these snippets with the `api` field on a ServiceTemplate or ServiceSpec endpoint to pass the scope through MeshAgent to the running container.
