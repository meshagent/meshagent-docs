...
import McpGenericAgentExample from "/snippets/examples/cli/generic-mcp-agent/meshagent.mdx"
...
**Add authentication** — If your MCP server requires OAuth, add an `oauth` configuration to the `mcp` endpoint. See the [`ports.endpoints`](./deploy_services#ports-endpoints) section in the field reference.
...
- [Supabase MCP Guide](../../agents/tools/supabase_mcp) — connect one Supabase project as a room toolkit.
- [MCP Servers](../../agents/tools/mcp_servers) — connect external MCP tools through Powerboards, `meshagent service`, `meshagent mcp`, or OpenAI connector manifests.

## Related Topics
- [Packaging & Deploying Services](./deploy_services) — Service configuration reference and key concepts.
- [MCP endpoint fields](./deploy_services#ports-endpoints) — Full field reference for MCP endpoint configuration.
- [Powerboards](../../interfaces/powerboards) — Install services from built-ins, install links, or MCP server URLs.
