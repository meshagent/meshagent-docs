...
> Typed values: MeshAgent also supports `json`, `uuid`, `list`, and `struct` database types. In the typed SDKs, use the wrapper classes for those values, such as `DatabaseJson`, `DatabaseStruct`, `DatabaseUuid` / `UuidValue`, and `DatabaseExpression` where the client requires them.
...
- **Parameters**:
  ...
  - **values**: A record of key-value pairs for direct assignment or expressions (e.g. `{ age: 30 }` or `{ age: new DatabaseExpression("age + 1") }` in typed clients).
...
await room.database.update(
  ...
  values={
    ...
    "loginCount": DatabaseExpression("loginCount + 1"),
  },
)
...
await room.database.update({
  ...
  values: {
    email: "newcharlie@example.com",
    loginCount: new DatabaseExpression("loginCount + 1"),
  },
});
