...
> **Note:** The adapter doesn’t “auto-register” these builders by default. Your agent decides which builders to expose each turn (e.g., `ChatBot.get_thread_tool_providers(...)`). This lets `ChatBot` substitute its thread-aware wrappers (like `ChatBotThreadOpenAIImageGenerationToolkitBuilder`).
