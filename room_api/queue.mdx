...
If a deployed service needs to use room queues, its participant token needs the appropriate queue-related Room API grants. These grants can be scoped to list/send/receive capabilities. See [API Scopes](../rest_api/api_scopes) and [Packaging and Deploying Services](../services_room_containers/packaging_and_deploying/packaging).
...
# Send a messages
await room.queues.send(name="my-queue", message={"payload": "Hello World!"})

# Receive a message
message = await room.queues.receive(name="my-queue")
...
// Send a message
await room.queues.send("my-queue", { payload: "Hello World!" }, { create: true });
