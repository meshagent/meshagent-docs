...
When an agent is deployed, it runs with a participant identity and a set of room/API grants. That means you can decide whether the agent should be able to read storage, write to the dataset, use secrets, run containers, or access other room capabilities. Deployment does not automatically grant every capability; the agent only gets the access you give it.
