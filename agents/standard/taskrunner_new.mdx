...
Most users should start with `LLMTaskRunner`, the built-in TaskRunner implementation that calls an LLM and optional tools. The CLI `meshagent task-runner` command spins up an `LLMTaskRunner` by default, and MeshAgent also ships prebuilt LLMTaskRunners like `meshagent.runner` and `meshagent.schema_planner` which are automatically available in every room.
...
### How to think about TaskRunner
- **`TaskRunner` is the base contract.** It is intentionally thin so you can wrap any existing agent or custom logic. You define input/output schemas and implement `ask(...)`. The `ask()` method can perform any operation you require whether that's invoking an agent from another framework, performing a business operation, calling an API, etc. 
- **`LLMTaskRunner` is the default implementation.** It fulfills the base contract by calling an LLM adapter, resolving tools, and returning model output. This allows you to use an LLM for single-turn tasks instead of conversational ones. 
- **MeshAgent built-ins are LLMTaskRunners.** The prebuilt agents (`meshagent.runner`, `meshagent.schema_planner`) are LLM-based TaskRunners available in every room. The CLI TaskRunner is also based on the `LLMTaskRunner`. 
...
- **Accepts a JSON schema input** and optionally enforces an output schema
- **Can be invoked by other agents** (for example a ChatBot calling a TaskRunner as a tool)
- **Needs tools for a single turn** without maintaining a chat thread

If you need a multi-turn conversation, use [ChatBot](./chatbot). For real-time speech, use [VoiceBot](./voicebot). For background queue work, use [Worker](./worker) or [MailWorker](./mailworker).

## Using built-in TaskRunners 
MeshAgent ships LLM-based TaskRunners that are already available in every room:
- `meshagent.runner`: This agent returns a text response. You provide it a prompt and optional tools. This is great for single-turn tasks and testing how your custom tools perform with an agent. 
- `meshagent.schema_planner`: Returns a structured output matching a schema you provide
...
This example shows how to expose an `LLMTaskRunner` (and a dynamic variant) using `ServiceHost`.
...
### Step 3: Run a task in MeshAgent Studio
1. Go to [MeshAgent Studio](https://www.studio.meshagent.com) and log in
2. Enter your room `quickstart`
3. Open the **Run Task...** menu and invoke your TaskRunner
...
- `LLMTaskRunner` is a concrete implementation that calls an LLM adapter, resolves toolkits, and returns model output.
...
- **Discovery:** Registration exposes your TaskRunner's schema and capabilities to Studio, CLI, and other agents.
...
## Next Steps
- [Prebuilt TaskRunners](./taskrunner_prebuilt) for `meshagent.runner` and `meshagent.schema_planner`
- [Custom LLM TaskRunner](./taskrunner_custom_llm)
