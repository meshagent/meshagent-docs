...
Now that our agent is working we can package and deploy it so it's always available for us to use! To do so we'll need to create a `meshagent.yaml` file which defines how our agent should run. If you are building your own custom agent you'll also need to create a Dockerfile and build and push your container. For the sake of this example, we can use an existing MeshAgent base image which already has the code to run our agent!
