...
A ``ServiceTemplate`` manifest defines what inputs a container accepts and how they map to environment variables. See the [packaging services](../services_room_containers/packaging) documentation for more details. Template substitutions use Jinja and only run on strings that start with `!template`.
...
- [Secrets API](./runtime_secrets)
- [Packaging and Deploying Services](../services_room_containers/packaging)
