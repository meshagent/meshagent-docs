...
If your service needs scheduled work, add a `meshagent.agent.schedule` annotation to the relevant agent in `meshagent.yaml` (cron or one-time). See the [packaging guide](./packaging#scheduled-tasks) for the annotation format and the [Scheduled Tasks API](../rest_api/overview#scheduled-tasks) for programmatic management.
