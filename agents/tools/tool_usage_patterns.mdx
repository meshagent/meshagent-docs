...
Every user message is a new turn, and the tool list is rebuilt for that turn by merging always-on toolkits with any dynamic toolkits requested for the message. On the next message, the process repeats with a fresh tool list.
...
- During the turn, `make_toolkits(room, model, providers, tools)` validates each config against the builder’s `ToolkitConfig` and instantiates only the requested toolkits.
...
```
This `message` dict is the payload your client sends to the agent; the key piece is the `tools` array, which must match the selected builder names and config schema.
...
3. **Agent turn**: `ChatBot` collects static thread toolkits, calls `make_toolkits` with the selected configs, and flattens the combined toolkit list.  
...
For MCP and OpenAI Connector toolkits, the adapter also reaches out to each MCP server during the request to discover that server's tool definitions.
