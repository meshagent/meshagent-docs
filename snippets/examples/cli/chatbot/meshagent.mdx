...
container:
  ...
  command: "/usr/bin/meshagent/meshagent chatbot service --agent-name=chatbot --image-generation=gpt-image-1 --require-storage --web-search --room-rules='agents/chatbot/rules.txt' --rule='You are a helpful assistant'"
  storage:
    room:
