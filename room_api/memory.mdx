...
The `MemoryClient` manages room-scoped structured memories. Use it to:

- List, create, inspect, and drop memories.
- Query and update memory datasets directly.
- Ingest text, files, images, tables, and storage content.
- Recall relevant entities and relationships for prompts and workflows.
- Clean up or optimize memory datasets.
...
> - **TypeScript/.NET**: no dedicated `MemoryClient` yet; use CLI or call toolkit `memory` via `room.invoke(...)` / `room.Invoke(...)`.
