...
Copy this code and activate your virtual environment then run the service: 

```bash bash
meshagent setup # this will prompt you to authenticate to MeshAgent, select your project and API keys
meshagent service run "main.py" --room survey
```

Next go to [studio.meshagent.com](https://studio.meshagent.com) and click into the ``survey``. You will need to have the Studio open to fill in the form. We can invoke the survey tool by invoking it from a separate tab in the terminal:
