...
Project secrets are **credentials managed at the project level**, often used by your long-running backend services. They’re not tied to any individual user, but to your project as a whole.

There are two main types of **project secrets**:
...
- You do not need per-user tokens during a live room session (use the [Secrets API](./secrets_overview) for this).
...
1. **Create a new secret**: 
    ```bash 
    meshagent room secret keys create --name weather-api-credentials --data WEATHER_API_KEY=xxx WEATHER_API_ENDPOINT=yyy 
    ```
    This will return the ID for this secret key. You can also return a list of secret names and IDs in your project by running ``meshagent room secret list``. Use the secret ID (not the name) in your manifest.
...
1. **Create an Image Pull Secret**: There are different subcommands for each registry. 
    ```bash 
    # Docker Hub
    meshagent room secret docker create --name docker-hub --server myregistry.docker.io --username user --password ****

    # Azure Container Registry
    meshagent room secret acr create --name my-acr --server myregistry.azurecr.io --username serviceprincipalid --password ****

    # GCP Artifact Registry
    meshagent room secret gar create --name gcp-registry --server us-docker.pkg.dev --json_key ****
    ```
...
A ``ServiceTemplate`` manifest defines what inputs a container accepts and how they map to environment variables. See the [packaging services](../services_room_containers/packaging_and_deploying/packaging) documentation for more details. Template substitutions use Jinja and only run on strings that start with `!template`.
...
environment: 
  ...
  - name: RESEARCH_SYSTEM_PROMPT 
    value: "!template {{ research_system_prompt }}"
  # App-supplied and injected by a project service at launch
  - name: RESEARCH_API_KEY     
    value: "!template {{ research_api_key }}"
  - name: GUARDRAIL_PROMPT 
    value: "!template {{ guardrail_prompt }}"
```
...
```bash 
meshagent room container run --image registry.example.com/tools:latest --env API_KEY=xxxx
```
...
- Audit unused secrets periodically (`meshagent room secret list` or Studio UI).
