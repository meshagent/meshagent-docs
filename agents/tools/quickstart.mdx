...
First write the logic for the `add` and `subtract` tools. Then bundle them into a `RemoteToolkit` that the `ServiceHost` can register for room calls.
...
```bash bash 
meshagent room agent invoke-tool --room=toolsroom --toolkit=math-toolkit --tool=add --arguments='{"a": 5, "b":7}'
```

#### Invoking tools from code

You can also call the tool directly from code. These examples connect to the room with a participant token and then invoke the `add` and `subtract` tools on `math-toolkit`.

Set `MESHAGENT_API_KEY` in your environment before running them.
...
```bash 
meshagent room agent list-toolkits --room=toolsroom
```

#### Discovering tools from code
Running this snippet lists the toolkits that are currently available in the room. Again the example connects with a participant token and then inspects the room's available toolkits.
...
- [Packaging and Deploying Services](../../services_room_containers/packaging_and_deploying/packaging): Package and deploy agents and tools as room or project services.
