...
Once you've built and pushed the container, add the service to a room using the MeshAgent CLI or MeshAgent Studio UI: 
**Using the MeshAgent CLI**
```bash bash
meshagent service create --file=meshagent.yaml --room=otel
```

**From MeshAgent Studio**
...
meshagent room agents invoke-tool \
  --room=otel \
  --require-toolkit=weather-toolkit \
  --tool=get_weather \
