...
* `service-id`: Render YAML with meshagent.service.id...
...
```console
$ meshagent service service-id [OPTIONS]
```
...
* `-f, --file TEXT`: File path to a service definition
* `--url TEXT`: URL to a service definition
* `--mcp TEXT`: MCP server URL. Auto-discovers metadata and generates a spec before adding meshagent.service.id.
* `--service-id TEXT`: Value for meshagent.service.id. Defaults to metadata.name when omitted.
* `--format [service|template]`: Output format. &#x27;service&#x27; emits Service YAML. &#x27;template&#x27; emits ServiceTemplate YAML.  [default: service]
* `--help`: Show this message and exit.
