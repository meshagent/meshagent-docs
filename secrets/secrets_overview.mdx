...
- **[Runtime secrets](./runtime_secrets)**: Collected at room runtime. These are per-user credentials (like OAuth tokens) managed by the **room Secrets API**. They’re tied to participants inside a room and may be refreshed or delegated between participants.

Both systems solve different problems but follow the same principle: least privilege, scoped access, secure storage.
...
- **Runtime Secrets API**: The interface (``room.secrets``) that allows agents or tools to request, store, refresh, and revoke participant-scoped credentials (including OAuth flows).
...
| A user’s OAuth token inside a room session                                | **Runtime Secrets API** |

## Next steps
- Learn how to define and use project-level credentials in [Project Secrets & Registry Access](./project_secrets).
- Learn how to request and manage participant credentials at runtime in [Runtime Secrets API](./runtime_secrets).
- See how services and room containers consume secrets in Services & Containers Overview[Services & Containers Overview](../services_room_containers/overview). 