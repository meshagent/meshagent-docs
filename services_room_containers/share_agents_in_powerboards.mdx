...
import PowerboardsChatbotCustomRule from "/snippets/examples/cli/concepts/use-powerboards-custom-rule/meshagent.mdx"
...
2. **Deploy agents in projects/rooms you're a member of with the MeshAgent CLI** 
   - The CLI allows you to deploy agents directly with commands like `meshagent process deploy...` or from a configuration file using commands like `meshagent service create...` for static configurations or `meshagent service create-template...` for dynamic configurations (e.g. if a user needs to provide an email for an agent).
...
Here's a simple example — a process-backed agent with a web search tool that users can customize with an initial rule when they install it:
...
## Related topics
- [Packaging and Deploying Services](./packaging_and_deploying/packaging): Learn how to create a YAML file to package your service and deploy it with the MeshAgent CLI.
- [CLI Quickstart](../introduction/cli_quickstart): Quickstart and CLI flags for `meshagent process`.
