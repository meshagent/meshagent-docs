...
## Overview
The MeshAgent **[ChatBot](./chatbot)** lets you add a conversational (chat/text based) agent to any MeshAgent Room with just a few lines of code!

In this quickstart we'll build the agent in four phases: 
1. **[Using the MeshAgent CLI](#phase-1-chat-with-the-built-in-chatbot-from-the-cli)**: With one command we can call a simple chatbot into a MeshAgent room. 
2. **[Basic ChatBot](#phase-2-building-a-simple-chatbot)**: A simple agent with a system prompt (rules). This allows us to see the code behind the CLI chatbot. 
3. **[ChatBot with Built-in MeshAgent Tools](#phase-3-adding-built-in-meshagent-tools-to-our-chatbot)**: Extend our chat agent by adding MeshAgent's prebuilt tools to interact with the user and write documents to the room.
4. **[ChatBot with Built-in MeshAgent Tools and Custom Tools](#phase-4-adding-custom-tools-to-our-chatbot)**: Add our own tools to the agent for use-case specific tasks. 
...
```sh
meshagent call agent --url=http://localhost:8081/chat --room=gettingstarted --participant-name=chatagent
```

**Note**: This will bring in the agent named ``chatagent`` into the ``gettingstarted`` room and run it on port ``8081`` using the service path ``chat``. If you want to rename the agent, room, port, or service path simply update the variables in the command.
