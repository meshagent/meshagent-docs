...
The ``MailBot`` lets your build an agent that communicates via email. It receives incoming messages, stores them in your room, generates replies using an LLM, and sends responses back out through SMTP — automatically turning your inbox into an intelligent, room-connected agent.

``MailBot`` extends the [``Worker``](./worker) agent to process inbound email from a queue and send replies via SMTP. It automatically persists every message and attachment to room storage, builds conversation context from past emails, and can call tools or reason over messages like any other agent.

> **Prerequisite**: To use ``MailBots`` you must either be an admin of the MeshAgent project you are using or have an admin create and allocate an email address for the agent to use. Only project admins can create and allocate email addresses. 

## When to Use It
Use ``MailBot`` when you want an agent that:
...
## Example
First create and allocate an available email address, then define a ``MailBot`` that uses it. 
...
At this point, the MailBot is connected to the ``maildemo`` room and listening on the queue you configured. As mail arrives (forwarded into the queue), the worker will parse, store, generate a reply, and send it via SMTP.
