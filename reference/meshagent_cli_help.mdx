...
* `delete`: Delete a project registry repository.
...
```console
$ meshagent  delete [OPTIONS] REPOSITORY_ID
```

**Arguments**:

* `REPOSITORY_ID`: Repository id to delete  [required]
...
* `pack`: Publish a local directory as an image...
...
* `--project-id TEXT`: A MeshAgent project id. If empty, the activated project will be used.
* `--room TEXT`: Room name
* `--tag TEXT`: Image tag to build. Must use the configured MeshAgent registry in the form &lt;registry&gt;/&lt;project-key&gt;/&lt;repository&gt;:&lt;tag&gt;.  [required]
* `--pack TEXT`: Local directory to stream as the build context. Format &#x27;&lt;path&gt;[:&lt;mount&gt;]&#x27;. Defaults mount to /context.  [required]
* `--context-path TEXT`: Build context path inside the streamed build context (absolute path). Defaults to the pack mount path.
...
* `--pack TEXT`: Stream a local directory as the build context, build the image, and then deploy it. Format &#x27;&lt;path&gt;[:&lt;mount&gt;]&#x27;. Defaults mount to /context.
...
Publish a local directory as an image inside a room using a generated Dockerfile.
...
* `PATH`: Local directory to publish  [required]

**Options**:

* `--project-id TEXT`: A MeshAgent project id. If empty, the activated project will be used.
* `--room TEXT`: Room name
* `--tag TEXT`: Image reference to publish. Must use the configured MeshAgent registry in the form &lt;registry&gt;/&lt;project-key&gt;/&lt;repository&gt;:&lt;tag&gt;.  [required]
* `--base TEXT`: Optional base image reference for the generated Dockerfile.
* `--help`: Show this message and exit.
